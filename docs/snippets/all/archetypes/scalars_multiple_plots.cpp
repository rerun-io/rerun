...
int main(int argc, char* argv[]) {
    const auto rec =
        rerun::RecordingStream("rerun_example_scalar_multiple_plots");
    ...
    rec.log_static(
        "scatter/lcg",
        rerun::SeriesPoints().with_markers(rerun::MarkerShape::Circle)
    );
    ...
    for (int t = 0; t < static_cast<int>(TAU * 2.0 * 100.0); ++t) {
        ...
        lcg_state = (1140671485 * lcg_state + 128201163) %
