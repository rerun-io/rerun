...
def read_h264_samples_from_rrd(
    ...
    # For that we just read out the first codec value batch and check whether
    # it's H.264.
    first_codec_batch = (
        df.select(f"/{video_entity}:VideoStream:codec").execute_stream().next()
    )
    if first_codec_batch is None:
        raise ValueError(
            f"There's no video stream codec specified at {video_entity} "
    ...
    if codec_value != rr.VideoCodec.H264.value:
        ...
        raise ValueError(
            ...
            f"timeline {timeline}. "
            f"Got {hex(codec_value)}, but the value for H.264 is {h264}."
        )
    else:
        print(
            f"Video stream codec is H.264 at {video_entity} "
            f"for timeline {timeline}."
        )

    # Get the video stream
    timestamps_and_samples = df.select(
        timeline, f"/{video_entity}:VideoStream:sample"
    ).to_arrow_table()
...
def mux_h264_to_mp4(
    times: ChunkedArray, samples: ChunkedArray, output_path: str
) -> None:
    ...
    for packet, time in zip(
        input_container.demux(input_stream), times, strict=False
    ):
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Query video stream from a recording and mux it to an mp4 "
            "video file."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Video entity path to query (default: video_stream)",
    )
    parser.add_argument(
        "--timeline",
        type=str,
        default="time",
        help="Name of the timeline to query (default: time)",
    )
