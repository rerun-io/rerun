...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let rec = rerun::RecordingStreamBuilder::new("rerun_example_audio_clip")
        .spawn()?;
    ...
    let samples =
        Array2::from_shape_fn((sample_count, 2).f(), |(i, channel)| {
    ...
    let samples = rerun::datatypes::TensorData::try_from(samples)?
        .with_dim_names(["sample", "channel"]);

    rec.log(
        "audio",
        &rerun::AudioClip::new(samples, sample_rate)
            .with_channel_names(["left", "right"]),
    )?;
