...
arrow::Status run_main() {
    ...
    for (int64_t i = 0; i < STEPS; i++) {
        ARROW_RETURN_NOT_OK(
            one_per_timestamp_builder.Append(sin(static_cast<double>(i) / 10.0))
        );
    }
    ...
    auto one_per_timestamp = one_per_timestamp_result.value_or_throw();

    arrow::DoubleBuilder ten_per_timestamp_builder;
    for (int64_t i = 0; i < STEPS * 10; i++) {
        ARROW_RETURN_NOT_OK(ten_per_timestamp_builder.Append(
            cos(static_cast<double>(i) / 100.0)
        ));
    }
    ...
    auto ten_per_timestamp = ten_per_timestamp_result.value_or_throw();
