...
fn main() {
    let crate_path = Path::new(
        &re_build_tools::get_and_track_env_var("CARGO_MANIFEST_DIR")
    ...
    for subdir in fs::read_dir(&all_path)
        .unwrap_or_else(|err| panic!("Failed to read directory {all_path:?}: {err}"))
        .flatten()
    {
        ...
        for entry in fs::read_dir(subdir.path())
            .unwrap_or_else(|err| panic!("Failed to read directory {:?}: {err}", subdir.path()))
            .flatten()
        {
            let path = entry.path();
            if let Some(extension) = path.extension() {
                if extension == "rs" {
                    let snippet_name = path
                        .file_stem()
                    ...
                    let target_path = snippets_path.join(
                        path.file_name()
                            .expect("Path should have a file name"),
                    );
                    fs::copy(&path, &target_path)
                        .unwrap_or_else(|err| panic!("Failed to copy {path:?} to {target_path:?}: {err}"));
                }
            }
        }
    }
    ...
    re_build_tools::write_file_if_necessary(snippets_path.join("mod.rs"), source.as_bytes())
        .expect("Failed to write snippets/mod.rs");
}
