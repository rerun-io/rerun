...
In general, a recording is a semantic collection of related data, with associated metadata including as a recording ID and an application ID.
In their simplest form, recordings can be thought of individual `.rrd` files containing Rerun data (or, equivalently, a single [stream](../how-does-rerun-work.md#stream-to-viewer) of data generated by a logging process)..
