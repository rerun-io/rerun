...
## ⚠️ Breaking changes
### [`DepthImage`](/docs/reference/types/archetypes/depth_image) and [`SegmentationImage`](/docs/reference/types/archetypes/segmentation_image)
The `DepthImage` and `SegmentationImage` archetypes used to be encoded as tensors, but now they are encoded as blobs of bytes with an [`ImageFormat`](/docs/reference/types/components/image_format) consisting of a resolution and a datatype.
...
### [`Image`](/docs/reference/types/archetypes/image)
The `Image` and `SegmentationImage` archetypes used to be encoded as tensors, but now they are encoded as blobs of bytes with an [`ImageFormat`](/docs/reference/types/components/image_format) consisting of a resolution and a datatype.
...
### [`EncodedImage`](/docs/reference/types/archetypes/encoded_image)
`EncodedImage` is our new archetype for logging an image file, e.g. a PNG or JPEG.

#### Python
`rr.ImageEncoded` is deprecated. Image files (JPEG, PNG, …) should instead be logged with [`EncodedImage`](/docs/reference/types/archetypes/encoded_image),
...
For all of these, use [`EncodedImage`](/docs/reference/types/archetypes/encoded_image) instead.
...
When constructing a [`Mesh3D`](/docs/reference/types/archetypes/mesh3d):
...
### Overhaul of [`Transform3D`](/docs/reference/types/archetypes/Transform3D)
...
Instead, there are now several components for translation/scale/rotation/matrices that can live side-by-side in the [3D transform archetype](/docs/reference/types/archetypes/transform3d).
...
* [`Translation3D`](/docs/reference/types/components/translation3d)
* [`TransformMat3x3`](/docs/reference/types/components/transform_mat3x3)
* [`Scale3D`](/docs/reference/types/components/scale3d)
* [`RotationAxisAngle`](/docs/reference/types/components/rotation_axis_angle)
   * uses existing datatype with the same name
* [`RotationQuat`](/docs/reference/types/components/rotation_quat)
   * uses existing `Quaternion` datatype
* [`TransformRelation`](/docs/reference/types/components/transform_relation)
...
### `OutOfTreeTransform3D` removed in favor of [`InstancePoses3D`](/docs/reference/types/archetypes/instance_poses3d)
[`InstancePoses3D`](/docs/reference/types/archetypes/instance_poses3d) fulfills an extended role:
It works more similar to the [`Transform3D`](/docs/reference/types/archetypes/transform3d) archetype and is supported by all 3D spatial primitives.
...
### [`Boxes3D`](/docs/reference/types/archetypes/boxes3d) changes

`centers` is now a [`PoseTranslation3D`](/docs/reference/types/components/pose_translation3d) instead of a [`Position3D`](/docs/reference/types/components/position3d) component.
The main difference in behavior is that this means it overlaps with the newly introduced [`InstancePoses3D`](/docs/reference/types/archetypes/instance_poses3d) archetype.

`rotation` was removed in favor of `rotation_axis_angles` and `quaternions` which are
[`PoseRotationAxisAngle`](/docs/reference/types/components/pose_rotation_axis_angle) and `PoseRotationQuat`(/docs/reference/types/components/pose_rotation_quat) components.
