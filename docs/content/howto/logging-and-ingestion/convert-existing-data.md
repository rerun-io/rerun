...
We have a few [command line options](../../concepts/logging-and-ingestion/mcap/cli-reference.md) for converting MCAP data directly into an `RRD` if our built in dataloaders support all your message types.
However, the most general solution to support arbitrary message types the `logging` api.
...
* For example, a pose message maps to [`Transform3D`](../../reference/types/archetypes/transform3d.md), an image to [`Image`](../../reference/types/archetypes/image.md), point clouds to [`Points3D`](../../reference/types/archetypes/points3d.md).
...
Instead of logging directly, handlers extract data into collectors.
Collectors accumulate data during iteration and is sent in bulk after the loop.
