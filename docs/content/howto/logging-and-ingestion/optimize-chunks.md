...
[`LazyChunkStream.collect()`](https://ref.rerun.io/docs/python/stable/experimental/#rerun.chunk.LazyChunkStream) materializes the pipeline into a `ChunkStore`; passing an `OptimizationProfile` runs extra compaction passes tuned for a specific target. The two presets mirror the CLI's `--profile` values:
