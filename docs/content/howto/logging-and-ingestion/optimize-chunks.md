...
[`LazyChunkStream.collect()`](https://ref.rerun.io/docs/python/stable/common/experimental#rerun.experimental.LazyChunkStream.collect) materializes the pipeline into a `ChunkStore`; passing an `OptimizationProfile` runs extra compaction passes tuned for a specific target. The two presets mirror the CLI's `--profile` values:
