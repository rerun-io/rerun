...
def log_bar_chart() -> None:
    ...
    viewer.log_tensor("bar_chart", y)


def log_parabola() -> None:
    # Log a parabola as a time series
    for t in range(0, 1000, 10):
        viewer.set_time_sequence("frame_nr", t)
        ...
        viewer.log_scalar("curves/parabola", f_of_t, label="f(t) = (0.01t - 3)³ + 1", radius=radius, color=color)
...
def log_trig() -> None:
    ...
    for t in range(0, int(tau * 2 * 100.0)):
        ...
        viewer.log_scalar("trig/sin", sin_of_t, label="sin(0.01t)", color=[255, 0, 0])

        cos_of_t = cos(float(t) / 100.0)
        viewer.log_scalar("trig/cos", cos_of_t, label="cos(0.01t)", color=[0, 255, 0])
...
def log_segmentation() -> None:
    ...
    for t in range(0, 1000, 2):
        ...
        f_of_t = (2 * 0.01 * t) + 2
        color = [255, 255, 0]
        viewer.log_scalar("segmentation/line", f_of_t, color=color, radius=3.0)
        ...
        viewer.log_scalar("segmentation/samples", g_of_t, color=color, scattered=True, radius=radius)
