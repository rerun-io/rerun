...
Pass `--audio path/to/audio.mp3` to decode common compressed audio formats with `ffmpeg`; use `--audio-sample-rate` and `--audio-channels` to control the decoded PCM clip.
