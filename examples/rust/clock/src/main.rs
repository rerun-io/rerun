...
use rerun::{external::re_log, MsgSender, RecordingContext};
...
fn run(rec_ctx: &RecordingContext, args: &Args) -> anyhow::Result<()> {
    ...
    for step in 0..args.steps {
        #[rustfmt::skip]
        log_hand(rec_ctx, "seconds", step, (step % 60) as f32 / 60.0, LENGTH_S, WIDTH_S, 0)?;
        #[rustfmt::skip]
        log_hand(rec_ctx, "minutes", step, (step % 3600) as f32 / 3600.0, LENGTH_M, WIDTH_M, 128)?;
        #[rustfmt::skip]
        log_hand(rec_ctx, "hours", step, (step % 43200) as f32 / 43200.0, LENGTH_H, WIDTH_H, 255)?;
    }
...
fn main() -> anyhow::Result<()> {
    ...
    args.rerun
        .clone()
        .run("clock", default_enabled, move |rec_ctx| {
