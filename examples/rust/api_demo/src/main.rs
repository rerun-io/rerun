...
use rerun::{
    ...
    Component, ComponentName, EntityPath, MsgSender, RecordingContext,
};
...
fn demo_bbox(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
...
fn demo_extension_components(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
...
fn demo_log_cleared(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
    ...
    fn log_cleared(
        ...
        rec_ctx.record_path_op(timepoint.into(), PathOp::clear(recursive, ent_path.into()))?;
        Ok(())
    }
    ...
    // sim_time = 2
    log_cleared(rec_ctx, &sim_time(2 as _), "null_demo/rect/0", false)?;

    // sim_time = 3
    log_cleared(rec_ctx, &sim_time(3 as _), "null_demo/rect", true)?;
...
fn demo_3d_points(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
...
fn demo_rects(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
    use ndarray::prelude::*;
...
fn demo_segmentation(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
...
fn demo_text_logs(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
...
fn demo_transforms_3d(rec_ctx: &RecordingContext) -> anyhow::Result<()> {
    ...
    log_coordinate_space(rec_ctx, "transforms3d")?;
    log_coordinate_space(rec_ctx, "transforms3d/sun")?;
    log_coordinate_space(rec_ctx, "transforms3d/sun/planet")?;
    log_coordinate_space(rec_ctx, "transforms3d/sun/planet/moon")?;
    ...
    log_point(rec_ctx, "transforms3d/sun", 1.0, [255, 200, 10])?;
    log_point(rec_ctx, "transforms3d/sun/planet", 0.4, [40, 80, 200])?;
    log_point(
        rec_ctx,
...
fn run(rec_ctx: &RecordingContext, args: &Args) -> anyhow::Result<()> {
    ...
    for demo in demos {
        match demo {
            Demo::BoundingBox => demo_bbox(rec_ctx)?,
            Demo::ExtensionComponents => demo_extension_components(rec_ctx)?,
            Demo::LogCleared => demo_log_cleared(rec_ctx)?,
            Demo::Points3D => demo_3d_points(rec_ctx)?,
            Demo::Rects => demo_rects(rec_ctx)?,
            Demo::Segmentation => demo_segmentation(rec_ctx)?,
            Demo::TextLogs => demo_text_logs(rec_ctx)?,
            Demo::Transforms3D => demo_transforms_3d(rec_ctx)?,
        }
    }
...
fn main() -> anyhow::Result<()> {
    ...
    args.rerun
        .clone()
        .run("api_demo_rs", default_enabled, move |rec_ctx| {
