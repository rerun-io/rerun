//! Example app that opens a Rerun Viewer with the state timeline view showing test state data.
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let app_env = re_viewer::AppEnvironment::Custom("State timeline view example".to_owned());

    // Log some state change data via SDK so the state timeline view has something to show.
    log_state_data()?;
