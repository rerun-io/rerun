...
use rerun::{
    ...
    Archetype, ArchetypeName, Component, ComponentList, ComponentName, Datatype, Loggable,
...
impl Archetype for CustomPoints3D {
    ...
    fn as_component_lists(&self) -> Vec<&dyn ComponentList> {
        self.points3d
            ...
            .into_iter()
            .chain(
                [self.confidences.as_ref().map(|v| v as &dyn ComponentList)]
                    .into_iter()
                    .flatten(),
            )
            .collect()
...
/// A custom [`Component`] that is backed by a builtin [`Float32`] scalar [`Datatype`].
