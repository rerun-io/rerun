...
fn log_ar_frame(
    ...
    log_video_frame(rec_ctx, ar_frame)?;

    if let Some(ar_camera) = ar_frame.data.camera.as_ref() {
        log_ar_camera(rec_ctx, ar_frame.timepoint.clone(), ar_camera)?;
    }

    if let Some(points) = ar_frame.data.raw_feature_points.as_ref() {
        log_feature_points(rec_ctx, ar_frame.timepoint.clone(), points)?;
    }

    if let Some(&annotations) = annotations.0.get(&ar_frame.index) {
        log_frame_annotations(rec_ctx, &ar_frame.timepoint, annotations)?;
    }
...
fn log_video_frame(rec_ctx: &RecordingContext, ar_frame: &ArFrame) -> anyhow::Result<()> {
...
fn run(rec_ctx: &RecordingContext, args: &Args) -> anyhow::Result<()> {
    ...
    log_coordinate_space(rec_ctx, "world", "RUB")?;
    log_coordinate_space(rec_ctx, "world/camera", "RDF")?;

    log_baseline_objects(rec_ctx, &annotations.objects)?;
    ...
    'outer: loop {
        ...
        for (idx, ar_frame) in ar_frames.enumerate() {
            ...
            log_ar_frame(rec_ctx, &annotations, &ar_frame)?;
...
fn main() -> anyhow::Result<()> {
    ...
    args.rerun
        .clone()
        .run("objectron_rs", default_enabled, move |rec_ctx| {
