...
use rerun::{
    components::{LegacyColor, LineStrip3D, Point3D, Radius, Transform3D, Vec3D},
...
fn run(rec_stream: &RecordingStream) -> Result<(), MsgSenderError> {
    ...
    MsgSender::new("dna/structure/left")
        ...
        .with_component(&colors1.iter().copied().map(LegacyColor::from).collect_vec())?
        .with_splat(Radius(0.08))?
        .send(rec_stream)?;

    MsgSender::new("dna/structure/right")
        .with_component(&points2.iter().copied().map(Point3D::from).collect_vec())?
        .with_component(&colors2.iter().copied().map(LegacyColor::from).collect_vec())?
        .with_splat(Radius(0.08))?
        .send(rec_stream)?;
    ...
    MsgSender::new("dna/structure/scaffolding")
        ...
        .with_splat(LegacyColor::from([128, 128, 128, 255]))?
        .send(rec_stream)?;
