...
file with Rerun (`rerun file.cpp`).
    ...
    options.add_options()
      ...
      ("opened-application-id", "Currently opened application ID in the viewer, if any", cxxopts::value<std::string>())
      ("recording-id", "Optional recommended ID for the recording", cxxopts::value<std::string>())
      ("opened-recording-id", "Currently opened recording ID in the viewer, if any", cxxopts::value<std::string>())
