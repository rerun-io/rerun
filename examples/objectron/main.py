...
def log_annotated_bboxes(bboxes: Iterable[Object]) -> None:
    ...
    for bbox in bboxes:
        ...
        rr.log_obb(
            f"world/objects/baseline/box-{bbox.id}",
            bbox.scale,
            bbox.translation,
            rot.as_quat(),
    ...
    for frame_ann in frame_annotations:
        ...
        for obj_ann in frame_ann.annotations:
            ...
            if len(keypoint_pos2s) == 9:
                log_projected_bbox(f"world/camera/video/objects/box-{obj_ann.object_id}", keypoint_pos2s)
            else:
                for (id, pos2) in zip(keypoint_ids, keypoint_pos2s):
                    rr.log_point(
                        f"world/camera/video/objects/box-{obj_ann.object_id}/{id}",
                        pos2,
