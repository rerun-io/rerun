...
def test_points2d() -> None:
    points_arrays: list[rr_dt.Point2DArrayLike] = [
        [],
        np.array([]),
    ...
    for points, radii, colors, labels, draw_order, class_ids, keypoint_ids, instance_keys in all_arrays:
        ...
        points = cast(Optional[rr_dt.Point2DArrayLike], points)
        ...
        assert arch.points == rr_cmp.Point2DArray.from_similar([[1.0, 2.0], [3.0, 4.0]] if non_empty(points) else [])
