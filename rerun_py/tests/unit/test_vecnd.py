...
def assert_correct_vec2d(vec: rr.dt.Vec2D | None) -> None:
...
def assert_correct_vec3d(vec: rr.dt.Vec3D | None) -> None:
    assert vec is not None
...
def assert_correct_vec4d(vec: rr.dt.Vec4D | None) -> None:
    assert vec is not None
...
def test_vec2d(data: rr.dt.Vec2DLike) -> None:
    vec = rr.dt.Vec2D(data)
    assert_correct_vec2d(vec)


@pytest.mark.parametrize("data", VEC_3D_INPUT)
def test_vec3d(data: rr.dt.Vec3DLike) -> None:
    vec = rr.dt.Vec3D(data)
    assert_correct_vec3d(vec)


@pytest.mark.parametrize("data", VEC_4D_INPUT)
def test_vec4d(data: rr.dt.Vec4DLike) -> None:
    vec = rr.dt.Vec4D(data)
