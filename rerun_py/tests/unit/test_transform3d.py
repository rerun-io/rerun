...
def assert_correct_rotation3d(rot: rr.dt.Rotation3D | None) -> None:
    ...
    if isinstance(rot.inner, rr.dt.Quaternion):
        ...
        # TODO(ab): np.array-typed fields should be provided with a `eq` method that uses `np.all`
...
def test_translation_rotation_translation(input: rr.dt.Vec3DLike) -> None:
    ...
    trs = rr.dt.TranslationRotationScale3D(translation=input, from_parent=False)
...
def test_translation_rotation_scale(input: rr.dt.Scale3DLike) -> None:
    # TODO(ab): from_parent should be optional
    trs = rr.dt.TranslationRotationScale3D(scale=input, from_parent=False)
    assert_correct_scale3d(trs.scale)

    # TODO(ab): from_parent should be optional
    trs = rr.dt.TranslationRotationScale3D(scale=rr.dt.Scale3D(input), from_parent=False)
    assert_correct_scale3d(trs.scale)
...
def test_translation_rotation_rotation(input: rr.dt.Rotation3DLike) -> None:
    # TODO(ab): from_parent should be optional
    trs = rr.dt.TranslationRotationScale3D(rotation=input, from_parent=False)
    assert_correct_rotation3d(trs.rotation)

    # TODO(ab): from_parent should be optional
    trs = rr.dt.TranslationRotationScale3D(rotation=rr.dt.Rotation3D(input), from_parent=False)
    assert_correct_rotation3d(trs.rotation)


@pytest.mark.parametrize("trans", VEC_3D_INPUT)
@pytest.mark.parametrize("mat", MAT_3X3_INPUT)
def test_translation_and_mat3x3(trans: rr.dt.Vec3DLike, mat: rr.dt.Mat3x3Like) -> None:
    # TODO(ab): from_parent should be optional
    tm = rr.dt.TranslationAndMat3x3(translation=trans, matrix=mat, from_parent=False)
...
# TODO(ab): clean me
# def test_playground():
#     print(rr.dt.TranslationRotationScale3D(translation=[1, 3, 4]))
