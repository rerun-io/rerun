...
def test_spherical_harmonics3_accepts_coefficient_major_shapes() -> None:
    ...
    for shape in [(15, 3), (45,), (4, 15, 3), (4, 45)]:
        ...
        batch = rr.datatypes.SphericalHarmonics3Batch(values)
        assert len(batch.as_arrow_array()) == max(int(np.prod(shape)) // 45, 1)


def test_spherical_harmonics3_rejects_channel_major() -> None:
    """A channel-major `(3, 15)` array has 45 elements too — it must not be silently transposed."""
    rr.set_strict_mode(True)

    with pytest.raises(ValueError):
        rr.datatypes.SphericalHarmonics3Batch(np.zeros((3, 15), dtype=np.float16))


def test_spherical_harmonics3_rejects_unpadded_lower_degree() -> None:
    """Degree 2 is 8 coefficients per channel; it must be zero-padded to 15, not regrouped."""
    rr.set_strict_mode(True)

    with pytest.raises(ValueError):
        rr.datatypes.SphericalHarmonics3Batch(np.zeros((45, 8, 3), dtype=np.float16))
