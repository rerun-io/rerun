...
fn array_to_rust(arrow_array: &PyAny, name: Option<&str>) -> PyResult<(Box<dyn Array>, Field)> {
    ...
    unsafe {
        ...
        if let Some(name) = name {
            if name == <re_components::Tensor as re_log_types::Component>::legacy_name() {
                field.data_type = <re_components::Tensor as re_log_types::external::arrow2_convert::field::ArrowField>::data_type();
            } else if name == <re_components::Rect2D as re_log_types::Component>::legacy_name() {
                field.data_type = <re_components::Rect2D as re_log_types::external::arrow2_convert::field::ArrowField>::data_type();
            } else if name == <re_components::Transform3D as re_log_types::Component>::legacy_name()
            {
