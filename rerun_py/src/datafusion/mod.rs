...
pub(crate) fn register(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    datafusion_module.add_function(wrap_pyfunction!(bounded_image_extraction_udf, &datafusion_module)?)?;
    datafusion_module.add_function(wrap_pyfunction!(depth_image_to_point_cloud_udf, &datafusion_module)?)?;
