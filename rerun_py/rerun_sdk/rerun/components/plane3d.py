...
class Plane3D(encodings.Plane3D, ComponentMixin):
    """
    **Component**: An infinite 3D plane represented by a unit normal vector and a distance.
    ...
    # Note: there are no fields here because Plane3D delegates to encodings.Plane3D


class Plane3DBatch(encodings.Plane3DBatch, ComponentBatchMixin):
