...
class PixelFormat(Enum):
    """
    **Encoding**: Specifieds a particular format of an [`archetypes.Image`][rerun.archetypes.Image].

    Most images can be described by a [`encodings.ColorModel`][rerun.encodings.ColorModel] and a [`encodings.ChannelDatatype`][rerun.encodings.ChannelDatatype],
    e.g. `RGB` and `U8` respectively.

    However, some image formats has chroma downsampling and/or
    use differing number of bits per channel, and that is what this [`encodings.PixelFormat`][rerun.encodings.PixelFormat] is for.
    ...
    Also known as just "gray". This is virtually identical to a 8bit luminance/grayscale (see [`encodings.ColorModel`][rerun.encodings.ColorModel]).
    ...
    If not for this range limitation/remapping, this is almost identical to 8bit luminace/grayscale (see [`encodings.ColorModel`][rerun.encodings.ColorModel]).
    """
