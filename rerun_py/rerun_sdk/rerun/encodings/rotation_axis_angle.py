...
def _rotation_axis_angle__axis__special_field_converter_override(x: encodings.Vec3DLike) -> encodings.Vec3D:
    if isinstance(x, encodings.Vec3D):
        return x
    else:
        return encodings.Vec3D(x)
...
class RotationAxisAngle(RotationAxisAngleExt):
    ...
    axis: encodings.Vec3D = field(converter=_rotation_axis_angle__axis__special_field_converter_override)
...
class RotationAxisAngleBatch(BaseBatch[RotationAxisAngleArrayLike]):
    ...
    def _native_to_pa_array(data: RotationAxisAngleArrayLike, data_type: pa.DataType) -> pa.Array:
        from rerun.encodings import AngleBatch, Vec3DBatch
