# DO NOT EDIT! This file was auto-generated by crates/build/re_types_builder/src/codegen/python/mod.rs
# Based on "crates/build/re_type_definitions/rerun/encodings/channel_count_pair.def.rs".

# You can extend this class by creating a "ChannelCountPairExt" class in "channel_count_pair_ext.py".

from __future__ import annotations

from collections.abc import Sequence
from typing import TYPE_CHECKING, Any

import pyarrow as pa
from attrs import define, field

from .. import encodings
from .._baseclasses import (
    BaseBatch,
)
from .channel_count_pair_ext import ChannelCountPairExt

__all__ = ["ChannelCountPair", "ChannelCountPairArrayLike", "ChannelCountPairBatch", "ChannelCountPairLike"]


def _channel_count_pair__channel_id__special_field_converter_override(x: encodings.UInt16Like) -> encodings.UInt16:
    if isinstance(x, encodings.UInt16):
        return x
    else:
        return encodings.UInt16(x)


def _channel_count_pair__message_count__special_field_converter_override(x: encodings.UInt64Like) -> encodings.UInt64:
    if isinstance(x, encodings.UInt64):
        return x
    else:
        return encodings.UInt64(x)


@define(init=False)
class ChannelCountPair(ChannelCountPairExt):
    """**Encoding**: A pair representing a channel ID and its associated message count."""

    def __init__(self: Any, channel_id: encodings.UInt16Like, message_count: encodings.UInt64Like) -> None:
        """
        Create a new instance of the ChannelCountPair encoding.

        Parameters
        ----------
        channel_id:
            The channel ID.
        message_count:
            The message count for this channel.

        """

        # You can define your own __init__ function as a member of ChannelCountPairExt in channel_count_pair_ext.py
        self.__attrs_init__(channel_id=channel_id, message_count=message_count)

    channel_id: encodings.UInt16 = field(converter=_channel_count_pair__channel_id__special_field_converter_override)
    # The channel ID.
    #
    # (Docstring intentionally commented out to hide this field from the docs)

    message_count: encodings.UInt64 = field(
        converter=_channel_count_pair__message_count__special_field_converter_override
    )
    # The message count for this channel.
    #
    # (Docstring intentionally commented out to hide this field from the docs)


if TYPE_CHECKING:
    ChannelCountPairLike = ChannelCountPair | tuple[encodings.UInt16Like, encodings.UInt64Like]
    """A type alias for any ChannelCountPair-like object."""
else:
    ChannelCountPairLike = Any

ChannelCountPairArrayLike = ChannelCountPair | Sequence[ChannelCountPairLike]
"""A type alias for any ChannelCountPair-like array object."""


class ChannelCountPairBatch(BaseBatch[ChannelCountPairArrayLike]):
    _ARROW_DATATYPE = pa.struct([
        pa.field("channel_id", pa.uint16(), nullable=False, metadata={}),
        pa.field("message_count", pa.uint64(), nullable=False, metadata={}),
    ])

    @staticmethod
    def _native_to_pa_array(data: ChannelCountPairArrayLike, data_type: pa.DataType) -> pa.Array:
        return ChannelCountPairExt.native_to_pa_array_override(data, data_type)
