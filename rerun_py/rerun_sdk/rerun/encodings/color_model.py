...
class ColorModel(ColorModelExt, Enum):
    """
    **Encoding**: Specifies what color components are present in an [`archetypes.Image`][rerun.archetypes.Image].

    This combined with [`encodings.ChannelDatatype`][rerun.encodings.ChannelDatatype] determines the pixel format of an image.
    """
...
class ColorModelBatch(BaseBatch[ColorModelArrayLike]):
    ...
    def _native_to_pa_array(data: ColorModelArrayLike, data_type: pa.DataType) -> pa.Array:
        ...
        pa_data = [ColorModel.auto(v).value if v is not None else None for v in data]  # type: ignore[redundant-expr]  # ty: ignore[not-iterable]
