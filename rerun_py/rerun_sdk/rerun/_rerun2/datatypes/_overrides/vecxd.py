...
def override_vec2d_native_to_pa_array_override(data: Vec2DArrayLike, data_type: pa.DataType) -> pa.Array:
...
def override_vec3d_native_to_pa_array_override(data: Vec3DArrayLike, data_type: pa.DataType) -> pa.Array:
    # TODO(ab): get rid of this once we drop support for Python 3.8. Make sure to pin numpy>=1.25.
    if NUMPY_VERSION < (1, 25):
        # Older numpy doesn't seem to support `data` in the form of [Point3D(1, 2), Point3D(3, 4)]
        # this happens for python 3.8 (1.25 supports 3.9+)
...
def override_vec4d_native_to_pa_array_override(data: Vec4DArrayLike, data_type: pa.DataType) -> pa.Array:
    # TODO(ab): get rid of this once we drop support for Python 3.8. Make sure to pin numpy>=1.25.
    if NUMPY_VERSION < (1, 25):
        # Older numpy doesn't seem to support `data` in the form of [Point3D(1, 2), Point3D(3, 4)]
        # this happens for python 3.8 (1.25 supports 3.9+)
