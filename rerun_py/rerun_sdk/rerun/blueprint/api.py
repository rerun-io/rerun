...
class View:
    ...
    def _log_to_stream(self, stream: RecordingStream) -> None:
        ...
        for path, components in self.overrides.items():
            override_path = f"{self.blueprint_path()}/ViewContents/individual_overrides/{path}"

            if hasattr(components, "as_component_batches"):
                stream.log(override_path, components, recording=stream)  # type: ignore[attr-defined]
            elif isinstance(components, Iterable):
                for component in components:
                    stream.log(override_path, component, recording=stream)  # type: ignore[attr-defined]
            else:
                raise TypeError(
                    f"Expected one or more objects implementing rerun.AsComponents or iterable of rerun.DescribedComponentBatch, "
                    f"but got {type(components)} instead."
                )
