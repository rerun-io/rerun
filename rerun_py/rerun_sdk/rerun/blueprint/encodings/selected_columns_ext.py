...
class SelectedColumnsExt:
    """Extension for [SelectedColumns][rerun.blueprint.encodings.SelectedColumns]."""

    def __init__(
        self: Any,
        columns: Sequence[blueprint_encodings.ComponentColumnSelectorLike | encodings.Utf8Like],
    ) -> None:
        """
        ...
        time_columns: list[encodings.Utf8] = []
        component_columns: list[blueprint_encodings.ComponentColumnSelector] = []

        for column in columns:
            if isinstance(column, blueprint_components.ComponentColumnSelector):
                component_columns.append(column)
            elif isinstance(column, encodings.Utf8):
                ...
                try:
                    ...
                    time_columns.append(encodings.Utf8(column))
            else:
    ...
    def native_to_pa_array_override(input_data: SelectedColumnsArrayLike, data_type: pa.DataType) -> pa.Array:
        ...
        from ...encodings import Utf8Batch
