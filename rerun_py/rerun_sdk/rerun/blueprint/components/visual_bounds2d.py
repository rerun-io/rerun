...
class VisualBounds2D(VisualBounds2DExt, encodings.Range2D, ComponentMixin):
    """
    **Component**: Visual bounds in 2D space used for `Spatial2DView`.
    ...
    # Note: there are no fields here because VisualBounds2D delegates to encodings.Range2D


class VisualBounds2DBatch(encodings.Range2DBatch, ComponentBatchMixin):
