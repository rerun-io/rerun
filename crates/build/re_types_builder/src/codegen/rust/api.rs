...
fn quote_struct(
    ...
    if let Some(forwarded_type) = forwarded_type(obj)
        && obj.kind == ObjectKind::Component
        // Because of the orphan rule, `WrapperComponent` can only be used properly in `re_types`.
        // Elsewhere, we can't do impls on concrete `WrapperComponent` (like implementing `Default` or similar).
        && obj.crate_name() == "re_types"
    {
        // TODO: have to check whether there's some attribute that we ignored. Just make all those an error.
        return quote_fully_forwarded_struct_component(reporter, objects, obj, &forwarded_type);
    }
...
fn quote_fully_forwarded_struct_component(
    reporter: &Reporter,
    objects: &Objects,
    obj: &Object,
    forwarded_type: &TokenStream,
) -> TokenStream {
    let Object { name, fqname, .. } = obj;

    let name = format_ident!("{}", name);
    let newtype = format_ident!("__{}Marker", name);
    let quoted_doc = quote_obj_docs(reporter, objects, obj);
    ...
    quote! {
        ...
        #quoted_doc
        pub type #name = crate::WrapperComponent<#forwarded_type, #newtype>;

        // Needed to enable tuple struct initialization which otherwise doesn't work for type aliases.
        #[allow(non_snake_case)]
        #[inline]
        pub const fn #name(v: #forwarded_type) -> #name {
            crate::WrapperComponent::<#forwarded_type, #newtype>(v, std::marker::PhantomData)
        }
        ...
        // This one is surprisingly hard to implement generically without
        // conflicting with `impl<T> std::convert::From<T> for T;`
        impl<T: Into<#forwarded_type>> From<T> for #name {
            fn from(v: T) -> Self {
                #name(v.into())
            }
        }
...
fn forwarded_type(obj: &Object) -> Option<TokenStream> {
...
fn quote_trait_impls_for_datatype_or_component(
    ...
    let forwarded_type = forwarded_type(obj);
