...
use re_viewer_context::{FallbackContext, QueryContext, auto_color_egui, typed_fallback_for};
...
pub fn collect_series_name(
    ...
    if series_names.len() < num_series {
        let fallback_name: String =
            typed_fallback_for::<components::Name>(query_ctx, fallback_ctx, name_descriptor)
                .to_string();
        if num_series == 1 {
