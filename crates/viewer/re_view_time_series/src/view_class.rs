    ...
    fn ui(
        ...
        if is_resetting {
                ...
                && (move_delta != egui::Vec2::ZERO || zoom_delta != egui::Vec2::ONE)
            {
                ...
                let new_view_time_range = re_types::blueprint::components::TimeRange(TimeRange {
                    start: re_types::datatypes::TimeRangeBoundary::Absolute(
                        re_types::datatypes::TimeInt(new_x_range.start() as i64 + time_offset),
                    ),
                    end: re_types::datatypes::TimeRangeBoundary::Absolute(
                        re_types::datatypes::TimeInt(new_x_range.end() as i64 + time_offset),
                    ),
                });
