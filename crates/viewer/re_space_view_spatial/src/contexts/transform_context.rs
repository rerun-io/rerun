...
impl ViewContextSystem for TransformContext {
    ...
    fn execute(
        ...
        while let Some(parent_path) = current_tree.path.parent() {
            ...
            let out_of_tree_transforms = match transform_at(
                current_tree,
                ctx.recording(),
                &time_query,
                ...
                Ok(TransformsAtEntity::None) => Vec::new(),
                Ok(TransformsAtEntity::TreeTransform(parent_from_child)) => {
                    reference_from_ancestor *= parent_from_child.inverse();
                    Vec::new()
                }
                Ok(TransformsAtEntity::OutOfTreeTransforms(out_of_tree_transforms)) => {
    ...
    fn gather_descendants_transforms(
        ...
        for child_tree in subtree.children.values() {
            ...
            let new_transform = match transform_at(
                ...
                Ok(TransformsAtEntity::None) => TransformInfo {
                    reference_from_entity: reference_from_parent,
                    out_of_tree_transforms: Vec::new(),
                    parent_pinhole: encountered_pinhole.clone(),
                },

                Ok(TransformsAtEntity::TreeTransform(parent_from_child)) => TransformInfo {
                    reference_from_entity: reference_from_parent * parent_from_child,
                    out_of_tree_transforms: Vec::new(),
                    parent_pinhole: encountered_pinhole.clone(),
                },
    ...
    pub fn reference_from_entity_ignoring_pinhole(
        ...
        if let (true, Some(parent)) = (
            ...
            self.reference_from_entity(&parent).map(|t| {
                // TODO(andreas): Store this in the transform info?
                if let TransformsAtEntity::TreeTransform(parent_from_child) =
                    query_and_resolve_transforms_at_entity(ent_path, entity_db, query)
                {
                    t * parent_from_child
                } else {
                    t
...
fn transform_at(
    ...
    if let Some((image_from_camera, camera_xyz)) = get_cached_pinhole(entity_path, entity_db, query)
    {
        ...
        Ok(match transforms_at_entity {
            TransformsAtEntity::None => TransformsAtEntity::TreeTransform(obj_from_2d_content),
            TransformsAtEntity::TreeTransform(world_from_obj) => {
                TransformsAtEntity::TreeTransform(world_from_obj * obj_from_2d_content)
            }
            TransformsAtEntity::OutOfTreeTransforms(out_of_tree_transform) => {
                re_log::error_once!(
                    "Can't handle pinhole transforms and out-of-tree transforms at the same time."
                );
                TransformsAtEntity::OutOfTreeTransforms(out_of_tree_transform)
            }
        })
    } else if transforms_at_entity.is_none()  // If there is any other transform, we ignore `DisconnectedSpace`.
            && entity_db
