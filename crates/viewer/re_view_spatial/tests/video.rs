...
#[test]
fn test_full_video_sampling() {
    let video_path = video_test_file_mp4(VideoCodec::H264, false);
    let data = std::fs::read(&video_path).unwrap();
    let video_data =
        VideoDataDescription::load_from_bytes(&data, "video/mp4", "test_h264_pipeline").unwrap();

    let buffers: re_video::StableIndexDeque<&[u8]> = std::iter::once(data.as_slice()).collect();

    let mut state = re_video::AnnexBStreamState::default();
    let mut idr_count = 0;
    ...
    for (sample_idx, sample) in video_data.samples.iter_indexed() {
        ...
        let converted = video_data
            .sample_data_in_stream_format(&chunk, Some(&mut state))
            .unwrap();

        if chunk.is_sync {
            idr_count += 1;

            // IDR frame should have SPS/PPS
        ...
        // All frames should have Annex B start codes
        assert!(
