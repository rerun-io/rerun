...
fn query_and_resolve_tree_transform_at_entity(
    ...
    if let Some(rotation_quat) = result.component_instance::<components::RotationAxisAngle>(0) {
        transform *= glam::Affine3A::from(rotation_quat);
    }
    if let Some(rotation_axis_angle) = result.component_instance::<components::RotationQuat>(0) {
