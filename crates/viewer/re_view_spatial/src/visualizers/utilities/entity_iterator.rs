    ...
    ) -> Result<(), ViewSystemExecutionError>,
{
    ...
    for (data_result, instruction) in query.iter_visualizer_instruction_for(system_identifier) {
        ...
        let results = data_result.query_archetype_with_history::<A>(ctx, query, instruction);
