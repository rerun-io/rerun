    ...
    fn execute(
        ...
        process_archetype::<Self, LineStrips2D, _>(
            ...
            |ctx, spatial_ctx, results| {
                ...
                let all_colors = results.iter_as(timeline, LineStrips2D::descriptor_colors().component);
                let all_radii = results.iter_as(timeline, LineStrips2D::descriptor_radii().component);
                let all_labels = results.iter_as(timeline, LineStrips2D::descriptor_labels().component);
                let all_class_ids = results.iter_as(timeline, LineStrips2D::descriptor_class_ids().component);
                let all_show_labels =
