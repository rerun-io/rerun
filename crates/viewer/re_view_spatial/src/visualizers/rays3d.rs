...
use re_sdk_types::archetypes::Rays3D;
...
#[derive(Default)]
pub struct Rays3DVisualizer;

impl Rays3DVisualizer {
    fn process_data<'a>(
        ...
        results_iter: impl Iterator<Item = Rays3DComponentData<'a>>,
    ) {
        ...
        for ent_data in results_iter {
            ...
            let radii = process_radius_slice(
                ...
                num_instances,
                ent_data.radii,
                Rays3D::descriptor_radii().component,
            );
            let colors = process_color_slice(
                ctx,
                Rays3D::descriptor_colors().component,
                num_instances,
                &annotation_infos,
                ent_data.colors,
            );
            ...
            let world_from_obj = ent_context
                .transform_info
                .single_transform_required_for_entity(entity_path, Rays3D::name())
                .as_affine3a();
            ...
            let instance_positions = {
                ...
                itertools::izip!(ent_data.vectors, origins).map(|(vector, origin)| {
                    glam::Vec3::from(origin.0) + glam::Vec3::from(vector.0) * 0.5
                })
            };
            ...
            data.ui_labels.extend(process_labels_3d(
                LabeledBatch {
                    ...
                    show_labels: ent_data.show_labels.unwrap_or_else(|| {
                        typed_fallback_for(ctx, Rays3D::descriptor_show_labels().component)
                    }),
...
impl IdentifiedViewSystem for Rays3DVisualizer {
...
impl VisualizerSystem for Rays3DVisualizer {
    ...
    fn execute(
        ...
        process_archetype::<Rays3D, _, _>(
            ...
            |ctx, spatial_ctx, results| {
                let all_vectors = results.iter_required(Rays3D::descriptor_vectors().component);
                ...
                let all_origins = results.iter_optional(Rays3D::descriptor_origins().component);
                let all_colors = results.iter_optional(Rays3D::descriptor_colors().component);
                let all_radii = results.iter_optional(Rays3D::descriptor_radii().component);
                let all_labels = results.iter_optional(Rays3D::descriptor_labels().component);
                let all_class_ids = results.iter_optional(Rays3D::descriptor_class_ids().component);
                let all_show_labels =
                    results.iter_optional(Rays3D::descriptor_show_labels().component);
