        ...
        process_archetype::<Self, Cylinders3D, _>(
            ...
            |ctx, spatial_ctx, results| {
                ...
                let all_length_chunks =
                    results.get_required_chunk(Cylinders3D::descriptor_lengths().component);
                if all_length_chunks.is_empty() {
                    return Ok(());
                }
                let all_radius_chunks =
                    results.get_required_chunk(Cylinders3D::descriptor_radii().component);
                ...
                let all_labels =
                    results.iter_as(timeline, Cylinders3D::descriptor_labels().component);
                let all_fill_modes =
                    results.iter_as(timeline, Cylinders3D::descriptor_fill_mode().component);
                let all_line_radii =
