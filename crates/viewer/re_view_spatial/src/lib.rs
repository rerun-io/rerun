...
use re_viewer_context::{ViewClass, ViewContext};
...
pub fn configure_background<V: ViewClass>(
    ctx: &ViewContext<'_>,
    ...
    let kind: BackgroundKind =
        background.component_or_fallback(ctx, view, &Background::descriptor_kind())?;
    ...
    match kind {
        ...
        BackgroundKind::SolidColor => {
            let color: Color =
                background.component_or_fallback(ctx, view, &Background::descriptor_color())?;
            Ok((None, color.into()))
        }
    }
}
