...
pub fn type_fallbacks(registry: &mut FallbackProviderRegistry) {
    ...
    registry.register_type_fallback_provider::<components::ViewCoordinates>(|_| {
        archetypes::Pinhole::DEFAULT_CAMERA_XYZ
    });
    registry.register_type_fallback_provider::<components::SeriesVisible>(|_| true.into());
    registry.register_type_fallback_provider::<components::Resolution>(|ctx| {
    ...
    registry.register_fallback_provider(&archetypes::BarChart::descriptor_abscissa(), |ctx| {
        ...
        if let Some(((_time, _row_id), tensor)) = ctx
            ...
            .latest_at_component::<components::TensorData>(
                ...
                ctx.query,
                &archetypes::BarChart::descriptor_values(),
            )
            && tensor.is_vector()
        {
