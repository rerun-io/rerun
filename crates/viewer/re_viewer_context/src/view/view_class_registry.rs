...
use crate::{
    IdentifiedViewSystem, IndicatedEntities, PerVisualizer, PerVisualizerInView, QueryContext,
...
impl ViewClassRegistry {
    ...
    /// Filters out visualizers that aren't active in a given view from the result of [`visualizable_entities_for_visualizer_systems`].
    ...
    ) -> PerVisualizerInView<VisualizableEntities> {
        let Some(view_class) = self.class_entry(view_class_identifier) else {
            return PerVisualizerInView::empty(view_class_identifier);
        };
