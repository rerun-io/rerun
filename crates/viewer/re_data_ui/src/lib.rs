...
pub fn sorted_component_list_for_ui(
    iter: impl IntoIterator<Item = ComponentDescriptor>,
) -> Vec<ComponentDescriptor> {
    let mut components: Vec<ComponentDescriptor> = iter.into_iter().collect();

    // Put indicator components first.
    // We then sort by the short name, as that is what is shown in the UI.
    components.sort_by_key(|desc| (!desc.is_indicator_component(), desc.short_name()));
