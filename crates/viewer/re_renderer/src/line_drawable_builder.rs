...
impl<'ctx> LineDrawableBuilder<'ctx> {
    ...
    /// Render the lines with alpha blending instead of as opaque geometry.
    ///
    /// The default (opaque) pipeline uses alpha to coverage for antialiasing, which doesn't work
    /// well when blending with existing content (See [`crate::BlendWithBackground`]).
    ///
    /// This switches to the alpha blending pipeline, which looks better when blended with existing
    /// backgrounds but is for now only suited for 2D views, since it doesn't handle depth ordering
    /// within draw calls correctly yet.
...
impl<'ctx> LineBatchBuilder<'_, 'ctx> {
    ...
    pub fn add_strip_2d(
        ...
        self.add_strip(points.map(|p| p.extend(0.0)))
            .flags(LineStripFlags::STRIP_FLAG_FORCE_ORTHO_SPANNING)
    }
