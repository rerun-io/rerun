...
fn colormap_srgba(which: u32, t_unsaturated: f32) -> vec4f {
    ...
    if which == COLORMAP_GRAYSCALE {
        ...
        return colormap_inferno_srgba(t);
    } else if which == COLORMAP_MAGMA {
        return colormap_magma_srgba(t);
    } else if which == COLORMAP_PLASMA {
        return colormap_plasma_srgba(t);
    } else if which == COLORMAP_TURBO {
        return colormap_turbo_srgba(t);
    } else if which == COLORMAP_VIRIDIS {
        return colormap_viridis_srgba(t);
    } else if which == COLORMAP_CYAN_TO_YELLOW {
        return colormap_cyan_to_yellow_srgba(t);
    } else if which == COLORMAP_SPECTRAL {
        return colormap_spectral_srgba(t);
    } else if which == COLORMAP_TWILIGHT {
        return colormap_twilight_srgba(t);
    } else if which == COLORMAP_RVIZ_MAP {
...
///
/// The input will be saturated to [0, 1] range.
fn colormap_linear(which: u32, t: f32) -> vec4f {
    let color = colormap_srgba(which, t);
    return vec4f(linear_from_srgb(color.rgb), color.a);
}
...
fn colormap_turbo_srgba(t: f32) -> vec4f {
...
/// normalized (it will be saturated no matter what).
fn colormap_viridis_srgba(t: f32) -> vec4f {
    ...
    return vec4f(c0 + t * (c1 + t * (c2 + t * (c3 + t * (c4 + t * (c5 + t * c6))))), 1.0);
}

/// Returns a gamma-space sRGBA in 0-1 range.
/// This is a polynomial approximation from Plasma color map, assuming `t` is
/// normalized (it will be saturated no matter what).
fn colormap_plasma_srgba(t: f32) -> vec4f {
...
/// normalized (it will be saturated no matter what).
fn colormap_magma_srgba(t: f32) -> vec4f {
...
/// normalized (it will be saturated no matter what).
fn colormap_inferno_srgba(t: f32) -> vec4f {
...
fn colormap_cyan_to_yellow_srgba(t: f32) -> vec4f {
    let u = t * 2. - 1.;
    return vec4f(saturate(vec3f(1. + 3. * u, (1. + 3. * u * u) , 1. - 3. * u) / 4.), 1.0);
}

/// Returns a gamma-space sRGBA in 0-1 range.
/// The input (`t`) must be in the 0-1 range.
/// This is a polynomial approximation from Spectral color map.
fn colormap_spectral_srgba(t: f32) -> vec4f {
...
fn colormap_twilight_srgba(t: f32) -> vec4f {
