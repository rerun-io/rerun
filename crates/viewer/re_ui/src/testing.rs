...
/// rasterizer, so on Linux CI a small `failed_pixel_count_threshold` absorbs isolated
...
pub fn default_snapshot_options_for_ui() -> SnapshotOptions {
    ...
    SnapshotOptions::new()
        .threshold(1.0)
        .failed_pixel_count_threshold(10) // we sometimes have a few wrong pixels in text rendering in egui for unknown reasons
}
...
pub fn default_snapshot_options_for_3d(viewport_size: Vec2) -> SnapshotOptions {
    ...
    SnapshotOptions::new()
        .threshold(threshold)
        .failed_pixel_count_threshold(strict_on_ci(10, max_broken_pixels))
}
