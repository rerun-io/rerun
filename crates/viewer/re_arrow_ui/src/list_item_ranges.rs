...
pub fn list_item_ranges(ui: &mut Ui, range: Span<usize>, item_fn: &mut dyn FnMut(&mut Ui, usize)) {
    ...
    while current < range.end() {
        ...
        let chunk_range = Span::from_start_end(current, chunk_end);
        let id = ui.unique_id().with(chunk_range);
