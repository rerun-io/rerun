...
pub fn viewer_harness(options: &HarnessOptions) -> Harness<'static, App> {
    ...
    let mut harness = Harness::builder().wgpu().with_size(window_size);
    ...
    let _unit: () =
        harness.with_options(re_ui::testing::default_snapshot_options_for_3d(window_size));

    harness.build_eframe(|cc| {
