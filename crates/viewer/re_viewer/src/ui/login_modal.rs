...
impl LoginModal {
    ...
    fn open(&mut self, ui: &mut egui::Ui) {
        match State::open(ui) {
    ...
    fn done(&mut self, ui: &mut egui::Ui) {
        if let Some(state) = &self.state
            && let Some(credentials) = state.done()
        {
            re_log::info!("{credentials:?}");
            self.state = None;
        ...
        pub fn open(ui: &mut egui::Ui) -> Result<Option<Self>, Error> {
            ...
            let on_storage_event = Closure::wrap(Box::new({
                ...
                move |e: web_sys::StorageEvent| {
                    ...
                    let Some(decoded_nonce) = String::from_utf8(decoded_nonce).ok_or_log_error()
                    else {
                        return;
                    };
                    ...
                    let Some(response) =
                        serde_json::from_slice::<AuthenticationResponse>(&response)
                            .ok_or_log_error()
                    else {
                        return;
                    };
