...
pub fn open_recording(
    ...
    let data_source = entity_db.data_source.as_ref().map(|v| match v {
        re_log_channel::SmartChannelSource::File(_) => Some("file"), // .rrd, .png, .glb, …
        re_log_channel::SmartChannelSource::RrdHttpStream { .. } => Some("http"),
        re_log_channel::SmartChannelSource::RedapGrpcStream { .. } => None,
        re_log_channel::SmartChannelSource::MessageProxy { .. } => Some("grpc"),
        // vvv spawn(), connect() vvv
        re_log_channel::SmartChannelSource::RrdWebEventListener => Some("web_event"),
        re_log_channel::SmartChannelSource::JsChannel { .. } => Some("javascript"), // mediated via rerun-js
        re_log_channel::SmartChannelSource::Sdk => Some("sdk"),                     // show()
        re_log_channel::SmartChannelSource::Stdin => Some("stdin"),
    })?;
