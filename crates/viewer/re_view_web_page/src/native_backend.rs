...
pub(crate) fn go_back(view_id: ViewId) {
    with_webview(view_id, NativeWebView::go_back);
}

pub(crate) fn go_forward(view_id: ViewId) {
    with_webview(view_id, NativeWebView::go_forward);
}

pub(crate) fn reload(view_id: ViewId) {
    with_webview(view_id, NativeWebView::reload);
}
...
fn with_webview(view_id: ViewId, f: impl FnOnce(&NativeWebView) -> Result<(), NativeWebViewError>) {
    NATIVE_WEBVIEWS.with_borrow(|webviews| {
        if let Some(webview) = webviews.get(&view_id) {
            match f(webview) {
                Ok(()) | Err(_) => {}
            }
        }
