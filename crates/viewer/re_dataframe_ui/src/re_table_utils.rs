...
pub const CELL_SEPERATOR_STROKE_OFFSET: f32 = 0.5;
...
pub fn header_ui<R>(
    ...
    if !connected_to_next_cell {
        ui.painter().vline(
            rect.max.x - CELL_SEPERATOR_STROKE_OFFSET,
            rect.y_range(),
    ...
    ui.painter().hline(
        rect.x_range(),
        rect.max.y - CELL_SEPERATOR_STROKE_OFFSET, // - 1.0 prevents it from being overdrawn by the following row
        Stroke::new(1.0, ui.tokens().table_header_stroke_color),
    );
