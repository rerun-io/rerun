...
use arrow2::Either;
use re_log_types::{
    DataCellColumn, DataRow, DataTable, ErasedTimeVec, RowId, RowIdVec, TableId, TimePoint,
    TimeRange, Timeline,
};
...
impl DataStore {
    ...
    // TODO: how does that get loaded back though...?
    fn dump_static_tables(&self) -> impl Iterator<Item = DataTable> + '_ {
        self.static_tables.values().map(|static_table| {
            ...
            let rows = cells_per_row_id.into_iter().map(|(row_id, static_cells)| {
                DataRow::from_cells(
                    row_id,
                    TimePoint::timeless(),
                ...
                // TODO: pretty sure this is 100% safe but
                .unwrap()
            });
