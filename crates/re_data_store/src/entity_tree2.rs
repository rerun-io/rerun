...
// TODO: let's storeviewify this one too and call it a day!
pub struct EntityTree {
    ...
    // TODO: these 2 form a pair, and are also used upstairs: registration!
    /// When do we or a child have data?
    ///
    /// Data logged at this exact path or any child path.
    // TODO: let's try to get this one out just to see how it looks
    // TODO: ah, it covers all children too..!
    ...
    // TODO: we could get this one out if we wanted too.
    pub components: BTreeMap<ComponentName, ComponentStats>,

    // TODO
    pending_clears: Vec<(RowId, TimePoint)>,
}

impl StoreView for EntityTree {
    fn name(&self) -> String {
        "rerun.store_view.EntityTree".into()
    }
    ...
    // TODO: this should be add_data_msg + add_path_op then
    fn on_events(&mut self, events: &[re_arrow_store::StoreEvent]) {
        ...
        for event in events {
            match event.diff {
                re_arrow_store::StoreDiff::TimelessDataAdded(_) => todo!(),
                re_arrow_store::StoreDiff::DataAdded(timeline, time, diff) => {
                    let leaf = self.create_subtrees_recursively(
                        diff.entity_path.as_slice(),
                        0,
                        Some((timeline, time)),
                    );
                    ...
                    let fields = leaf
                        .components
                        .entry(diff.component_name)
                        .or_insert_with(|| {
                            // If we needed to create a new leaf to hold this data, we also want to
                            // insert all of the historical pending clear operations
                            self.pending_clears
                                .extend(leaf.nonrecursive_clears.clone().into_iter());

                            Default::default()
                        });

                    fields.add(Some((timeline, time)));
                }
                re_arrow_store::StoreDiff::TimelessDataRemoved(_) => todo!(),
                re_arrow_store::StoreDiff::DataRemoved(_, _, _) => todo!(),
            }
        }
    }
}
...
impl EntityTree {
    ...
    fn create_subtrees_recursively(
        ...
        depth: usize,
        time: Option<(Timeline, TimeInt)>,
        // time_point: &TimePoint,
    ) -> &mut Self {
        if let Some((timeline, time)) = time {
            self.prefix_times
                .0
                .entry(timeline)
                .or_default()
                .increment(time.as_i64(), 1);
        } else {
        ...
        match full_path.get(depth) {
            ...
            // TODO: why is this called component???
            Some(component) => self
                .children
                ...
                .create_subtrees_recursively(full_path, depth + 1, time),
        }
    }
...
impl ComponentStats {
    ...
    pub fn add(&mut self, time: Option<(Timeline, TimeInt)>) {
        if let Some((timeline, time)) = time {
            // TODO: why is this not a method on TimeHistogramPerTimeline..?
            self.times
                .0
                .entry(timeline)
                .or_default()
