...
impl DataStore {
    ...
    pub fn insert_row(&mut self, row: &DataRow) -> WriteResult<StoreEvent> {
        ...
        let diff = if timepoint.is_timeless() {
            // We interpret unstamped data as static.
            ...
            let cells = row
                ...
                // TODO: it's a mess but whatever, it'll go away soon
                .chain(std::iter::once(&generated_cluster_cell).flatten())
                .collect_vec();

            for cell in &cells {
                // TODO: maybe compare rowids if it already exist?
