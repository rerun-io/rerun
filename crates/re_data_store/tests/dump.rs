...
use re_data_store::{
    ...
    AbsoluteTimeRange, DataStore, DataStoreStats, GarbageCollectionOptions, TimeInt, Timeline,
};
use re_log_types::{
...
fn data_store_dump_filtered_impl(store1: &mut DataStore, store2: &mut DataStore) {
    ...
    row_set.insert_tables(
        store1.to_data_tables((timeline_frame_nr, AbsoluteTimeRange::new(frame1, frame1)).into()),
    );
    // Dump frame2 from the first store.
    row_set.insert_tables(
        store1.to_data_tables((timeline_frame_nr, AbsoluteTimeRange::new(frame2, frame2)).into()),
    );
    // Dump frame3 from the first store.
    row_set.insert_tables(
        store1.to_data_tables((timeline_frame_nr, AbsoluteTimeRange::new(frame3, frame3)).into()),
    );
    // Dump frame3 _from the other timeline_, from the first store.
    // This will produce the same RowIds again!
    row_set.insert_tables(
        store1.to_data_tables((timeline_log_time, AbsoluteTimeRange::new(frame3, frame3)).into()),
    );
    // Dump frame4 from the first store.
    row_set.insert_tables(
        store1.to_data_tables((timeline_frame_nr, AbsoluteTimeRange::new(frame4, frame4)).into()),
    );
