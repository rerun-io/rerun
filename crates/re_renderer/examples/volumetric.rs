...
// TODO:
// - ColoredVolume
// - ScalarVolume (transfer functions!)

struct RenderVolumetric {
    checkerboard_tex: GpuTexture3DHandle,
...
impl framework::Example for RenderVolumetric {
    ...
    fn new(re_ctx: &mut re_renderer::RenderContext) -> Self {
        ...
        let checkerboard_tex = re_ctx.texture_manager_3d.create(
            &mut re_ctx.gpu_resources.textures,
            &Texture3DCreationDesc {
                label: "checkerboard".into(),
                data: &checkerboard,
    ...
    fn draw(
        ...
        let volume_draw_data = VolumeDrawData::new(
            re_ctx,
            &[Volume {
                pos_in_world: -self.checkerboard_size * 0.5, // TODO
                // pos_in_world: Vec3::ZERO,
                size: self.checkerboard_size,
                dimensions: self.checkerboard_dimensions,
                texture: self.checkerboard_tex.clone(),
            }],
        )
        .unwrap();
