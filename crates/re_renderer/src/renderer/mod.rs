...
/// for each of its `draw` invocations. (typically `RenderPipeline`s and `BindGroupLayout`s)
/// Any data that might be different per specific `draw` invocation is stored in
