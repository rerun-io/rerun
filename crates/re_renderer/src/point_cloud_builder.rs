    ...
    PerPointUserData: Default + Copy,
{
    ...
    #[inline]
    pub fn add_point(&mut self, position: glam::Vec3) -> PointsBuilder<'_, PerPointUserData> {
        self.add_points(std::iter::once(position))
    }
    ...
    #[inline]
    pub fn add_point_2d(&mut self, position: glam::Vec2) -> PointsBuilder<'_, PerPointUserData> {
        let z = self.0.next_2d_z;
        self.0.next_2d_z += Self::NEXT_2D_Z_STEP;
        self.add_points(std::iter::once(position.extend(z)))
    }
}
