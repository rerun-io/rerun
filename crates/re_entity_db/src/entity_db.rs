...
use re_data_store2::{
    DataStore2, DataStoreConfig2, GarbageCollectionOptions, GarbageCollectionTarget, StoreEvent2,
    StoreSubscriber2,
};
use re_log_types::{
...
impl EntityDb {
    ...
    pub fn num_rows(&self) -> u64 {
        self.data_store2.stats().total.total_num_rows
    }
    ...
    pub fn add_chunk(&mut self, chunk: &Arc<Chunk>) -> Result<(), Error> {
        let store_event = self.data_store2.insert_chunk(chunk)?;
    ...
    pub fn gc(&mut self, gc_options: &GarbageCollectionOptions) {
        ...
        let (store_events, stats_diff) = self.data_store2.gc(gc_options);
...
impl re_types_core::SizeBytes for EntityDb {
    ...
    fn heap_size_bytes(&self) -> u64 {
        ...
        self.data_store2().stats().total.total_size_bytes
    }
}
