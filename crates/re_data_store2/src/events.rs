...
/// The atomic unit of change in the Rerun [`DataStore2`].
///
/// A [`StoreEvent2`] describes the changes caused by the addition or deletion of a
/// [`re_log_types::DataRow`] in the store.
///
/// Methods that mutate the [`DataStore2`], such as [`DataStore2::insert_row`] and [`DataStore2::gc`],
/// return [`StoreEvent2`]s that describe the changes.
...
pub struct StoreEvent2 {
    /// Which [`DataStore2`] sent this event?
    ...
    ///
    /// When handling a [`StoreEvent2`], if this is the first time you process this [`StoreId`] and
...
/// Describes an atomic change in the Rerun [`DataStore2`]: a row has been added or deleted.
///
/// From a query model standpoint, the [`DataStore2`] _always_ operates one row at a time:
...
mod tests {
    ...
    use re_log_types::{
        ...
        EntityPath, RowId, TimeInt, TimePoint, Timeline,
    };
    use re_types_core::{ComponentName, Loggable as _};

    use crate::{DataStore2, GarbageCollectionOptions};
