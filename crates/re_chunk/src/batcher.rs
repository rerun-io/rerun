...
use crate::{chunk::ChunkResult, Chunk};
...
impl ChunkBatcherConfig {
    ...
    /// Environment variable to configure [`Self::flush_num_rows_if_unsorted`].
...
impl PendingRow {
    ...
    ///
    /// That's very wasteful, don't do that outside of testing.
    pub fn into_chunk(self, entity_path: EntityPath) -> ChunkResult<Chunk> {
        ...
        let timelines = timepoint
            .into_iter()
            .map(|(timeline, time)| (timeline, vec![time]))
            .collect();
        ...
        Chunk::new(entity_path, Some(true), vec![row_id], timelines, components)
    }
    ...
    pub fn many_into_chunks(
        ...
        per_timeline_set.into_values().flat_map(move |rows| {
            ...
            per_datatype_set.into_values().flat_map(move |rows| {
                ...
                let mut timelines: BTreeMap<Timeline, Vec<TimeInt>> = BTreeMap::default();
                let mut timelines_sorted: IntMap<Timeline, bool> = IntMap::default();
                ...
                for row in &rows {
                    ...
                    for (&timeline, &time) in row_timepoint {
                        let times = timelines.entry(timeline).or_default();
                        let sorted = timelines_sorted
                            .entry(timeline)
                            .and_modify(|sorted| *sorted &= times[times.len() - 1] > time)
                            .or_insert(true);
...
mod tests {
    ...
    fn simple() -> anyhow::Result<()> {
        ...
        chunks.sort_by_key(|chunk| chunk.first_row_id());
