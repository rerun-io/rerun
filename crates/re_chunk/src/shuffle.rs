...
impl Chunk {
    ...
    /// For most use cases, the small gains in cache locality are not worth the much higher sorting
    /// costs.
    #[inline]
    ...
    /// memory in order to make it contiguous.
    /// Otherwise, only the offsets will be shuffled.
    /// For most use cases, the small gains in cache locality are not worth the much higher
    /// shuffling costs.
    #[inline]
