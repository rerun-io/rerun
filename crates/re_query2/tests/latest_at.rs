...
fn simple_query() {
    ...
    let positions_row_id = RowId::new();
    let row =
        DataRow::from_cells1_sized(positions_row_id, entity_path, timepoint, 2, positions).unwrap();
    insert_and_react(&mut store, &mut caches, &row);

    let colors = vec![MyColor::from_rgb(255, 0, 0)];
    let colors_row_id = RowId::new();
    let row = DataRow::from_cells1_sized(colors_row_id, entity_path, timepoint, 1, colors).unwrap();
    insert_and_react(&mut store, &mut caches, &row);
...
fn query_and_compare(
    ...
    for _ in 0..3 {
        ...
        similar_asserts::assert_eq!(expected_points, cached_point_data);
        similar_asserts::assert_eq!(expected_colors, cached_color_data);
    }
}
