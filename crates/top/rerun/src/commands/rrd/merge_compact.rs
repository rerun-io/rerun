...
pub struct CompactCommand {
    ...
    #[arg(long = "num-pass", default_value_t = 8)]
    num_passes: u32,
...
fn merge_and_compact(
    ...
    // TODO: why are we building entity dbs instead of... stores?
    ...
    // TODO: then let's say it's not pass, but rather _extra_ pass
    let mut pass = 0;
    while pass < num_passes {
        ...
        entity_dbs = entity_dbs
            ...
            .map(|(store_id, db)| {
                ...
                for chunk in engine.read().store().iter_chunks() {
                    store.insert_chunk(chunk).unwrap(); // TODO
                }
        ...
        re_log::info!(pass, num_chunks_before, num_chunks_after, num_chunks_reduction, time=?now.elapsed(), "extra compaction pass completed");
    ...
    // TODO: do we just run until stable if not specified?

    // TODO: somehow do passes here
