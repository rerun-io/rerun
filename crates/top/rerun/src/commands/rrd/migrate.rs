...
fn migrate_from_to(from_path: &Utf8PathBuf, to_path: &Utf8PathBuf) -> anyhow::Result<()> {
    ...
    // TODO(ab): The `StoreId` migration in `Decoder` requires `SetStoreInfo` to arrive before the
    // corresponding `ArrowMsg`. Ideally this tool would cache orphan `ArrowMsg` until a matching
    // `SetStoreInfo` arrives.
