...
criterion_group!(benches, scalar_gc);
...
// TODO: for scalars, we care about...:
// - insertion perf
// - GC perf
// - range perf
//
// TODO: technically we'd also care about OOO setups etc, but leave that to external example or
// we'll never be done.

fn scalar_gc(c: &mut Criterion) {
...
fn build_store(config: DataStoreConfig, cluster_key: ComponentName, packed: bool) -> DataStore {
    ...
    // TODO: very important to intersperse them for realistic benchmark
    ...
    let mut table = DataTable::from_rows(
        ...
        (0..NUM_ROWS_PER_ENTITY_PATH).map(move |i| {
            DataRow::from_component_batches(
                RowId::random(),
                // TODO: multi-timeline is very important to test for here!
                [
