...
pub fn clean_for_polars(arr: Box<dyn Array>) -> Box<dyn Array> {
    match arr.data_type() {
        DataType::List(field) => {
            // Recursively clean the contents
            let typed_arr = arr.as_any().downcast_ref::<ListArray<i32>>().unwrap();
            let clean_vals = clean_for_polars(typed_arr.values().clone());
        ...
        DataType::LargeList(field) => {
            // Recursively clean the contents
            let typed_arr = arr.as_any().downcast_ref::<ListArray<i64>>().unwrap();
            let clean_vals = clean_for_polars(typed_arr.values().clone());
            let clean_data = DataType::List(Box::new(Field::new(
                &field.name,
                clean_vals.data_type().clone(),
        ...
        DataType::FixedSizeList(field, len) => {
            ...
            let typed_arr = arr.as_any().downcast_ref::<FixedSizeListArray>().unwrap();
            let clean_vals = clean_for_polars(typed_arr.values().clone());
            let clean_data = DataType::List(Box::new(Field::new(
                &field.name,
                clean_vals.data_type().clone(),
        ...
        DataType::Struct(fields) => {
            // Recursively clean the contents
            let typed_arr = arr.as_any().downcast_ref::<StructArray>().unwrap();
            let clean_vals = typed_arr
                .values()
                .iter()
                .map(|v| clean_for_polars(v.clone()))
                .collect_vec();
        ...
        DataType::Union(fields, ids, UnionMode::Dense) => {
            ...
            let typed_arr = arr.as_any().downcast_ref::<UnionArray>().unwrap();
            ...
            let padded_vals = itertools::izip!(&clean_vals, &ids)
                .map(|(dense, id)| {
                    let mut next = 0;
                    let mut grow = make_growable(&[dense.as_ref()], true, arr.len());
...
fn test_clean_for_polars_nomodify() {
    ...
    let cleaned = clean_for_polars(bundle.value.clone());
...
fn test_clean_for_polars_modify() {
    ...
    let cleaned = clean_for_polars(bundle.value);
