...
impl DataStore {
    ...
    /// Auto-generates an approriate cluster cell for the specified number of instances and
...
impl IndexedBucket {
    fn insert_row(
        ...
        *time_range = TimeRange::new(time_range.min.min(time), time_range.max.max(time));
        ...
        for cell in row.cells().iter() {
            ...
            column.0.push(Some(cell.clone() /* shallow */)); // TODO
        }
    ...
    fn split(&self) -> Option<(TimeInt, Self)> {
        ...
        let (min2, bucket2) = {
            ...
            let time_range2 = split_time_range_off(split_idx, col_time1, time_range1);

            // this updates `col_time1` in-place!
            let col_time2 = split_off_column(col_time1, split_idx);

            // this updates `col_insert_id1` in-place!
            let col_insert_id2 = split_off_column(col_insert_id1, split_idx);

            // this updates `col_row_id1` in-place!
            let col_row_id2 = split_off_column(col_row_id1, split_idx);

            // this updates `col_num_instances1` in-place!
            let col_num_instances2 = split_off_column(col_num_instances1, split_idx);

            // this updates `columns1` in-place!
            let columns2: IntMap<_, _> = columns1
                .iter_mut()
                .map(|(name, column1)| {
