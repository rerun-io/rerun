...
use re_sdk::sink::MemorySinkStorage;
...
where
    F: FnOnce(RecordingContext) + Send + 'static,
{
...
/// Starts a Rerun viewer to visualize the contents of a [`MemorySinkStorage`].
///
/// The method will return when the viewer is closed.
///
/// You should use this method together with [`RecordingContext::memory`];
///
/// ⚠️  This function must be called from the main thread since some platforms require that
/// their UI runs on the main thread! ⚠️
pub fn show(storage: &MemorySinkStorage) -> re_viewer::external::eframe::Result<()> {
    let msgs = storage.take();
    if msgs.is_empty() {
        re_log::debug!("Empty storage - call to show() ignored");
        return Ok(());
    }
