...
fn load_series(
    ...
    let query_caches2 = ctx.entity_db.query_caches2();
    let resolver = ctx.entity_db.resolver();
    ...
    // TODO: gentle reminder that we're still iterating over _everything_, irrelevant of the query.
    ...
    );
    {
        ...
        // TODO: for now we just fail if anything's pending -- in follow up PRs we'll handle
        // missing data on the edges.
        // TODO: not sure how one used to handle deser errors here..?
        ...
        // Allocate all points.
        points = all_scalars
            .indices(all_scalars_entry_range.clone())
        ...
        for (i, scalars) in all_scalars
            .data(all_scalars_entry_range.clone())
            .enumerate()
        {
            if scalars.len() > 1 {
                re_log::warn_once!(
        ...
        // TODO: we need a way to communicate that the thing is partially pending.
        ...
        if override_color.is_none() {
            if let Some(all_colors) = results.get(Color::name()) {
                ...
                let all_frames = re_query_cache2::range_zip_1x1(
                    all_scalars_indexed,
                    all_colors.indexed(query.range()),
                )
                .enumerate();
        ...
        if override_stroke_width.is_none() {
            if let Some(all_stroke_widths) = results.get(StrokeWidth::name()) {
                ...
                let all_frames = re_query_cache2::range_zip_1x1(
                    all_scalars_indexed,
                    all_stroke_widths.indexed(query.range()),
                )
                .enumerate();
