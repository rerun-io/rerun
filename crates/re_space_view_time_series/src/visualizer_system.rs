...
impl TimeSeriesSystem {
    fn load_scalars(
        ...
        // let plot_transform = plot_mem.as_ref().map(|mem| mem.transform());

        // // The width of the plot's canvas: i.e. how much space is it taking in the UI (in points)?
        // let plot_canvas_width = plot_transform
        //     .as_ref()
        //     .map_or(1.0, |plot_transform| plot_transform.frame().width() as f64);
        //
        ...
        for data_result in query.iter_visible_data_results(Self::identifier()) {
            ...
            let (aggregation_factor, points) = {
                ...
                // // Reduce the canvas width if the plot doesn't actually cover the whole canvas.
                // let mut plot_canvas_width = plot_canvas_width;
                // if let Some(plot_transform) = plot_transform {
                //     let left = plot_transform
                //         .position_from_point_x(points.first().map_or(f64::MIN, |p| p.time as f64))
                //         as f64;
                //     let right = plot_transform
                //         .position_from_point_x(points.last().map_or(f64::MAX, |p| p.time as f64))
                //         as f64;
                //
                //     let reduced_canvas_width = right - left;
                //     if reduced_canvas_width > 0.0 {
                //         plot_canvas_width = f64::min(plot_canvas_width, reduced_canvas_width);
                //     }
                // }
                ...
                let points = if aggregation_factor > 2.0 {
                    ...
                    match aggregator {
                        TimeSeriesAggregator::None => points,
                        ...
                        TimeSeriesAggregator::MinMaxMedian => {
                            MinMaxAggregator::MinMaxMedian.aggregate(aggregation_factor, &points)
                        }
                    }
                } else {
                    points
    ...
    fn aggregate(&self, aggregation_factor: f64, points: &[PlotPoint]) -> Vec<PlotPoint> {
        ...
        while i < points.len() {
                ...
                && are_aggregatable(&points[i], &points[i + j], window_size)
            {
                ...
                match self {
                    MinMaxAggregator::MinMax | MinMaxAggregator::MinMaxMedian => {
