...
use crate::{
    ...
    PlotPoint, PlotPoints, PlotSeries, PlotSeriesKind, ScatterAttrs,
};
...
pub fn points_to_series_soa(
    ...
    //     apply_aggregation_soa(aggregator, time_per_pixel, points, query);
    let min_time = store
        .entity_min_time(&query.timeline, &data_result.entity_path)
        .map_or(points.times.first().copied().unwrap_or(0), |time| {
            time.as_i64()
        });

    let same_label = |points: &PlotPoints| -> Option<Utf8> {
        let label = points.labels[0].as_ref()?;
        (points
            .labels
            .iter()
            .all(|label| label.as_ref() == label.as_ref()))
        .then(|| label.clone())
    };
    let series_label =
        same_label(&points).unwrap_or_else(|| data_result.entity_path.to_string().into());
    if points.len() == 1 {
        ...
        all_series.push(PlotSeries {
            label: series_label,
            color: points.colors[0],
            width: 2.0 * points.stroke_widths[0],
            kind,
            points: vec![(points.times[0], points.values[0])],
            entity_path: data_result.entity_path.clone(),
            aggregator,
...
fn add_series_runs_soa(
    ...
    let num_points = points.len();
    let mut cur = points.get(0);
    let mut series: PlotSeries = PlotSeries {
        label: series_label.clone(),
        color: cur.attrs.color,
        width: 2.0 * cur.attrs.stroke_width,
        points: Vec::with_capacity(num_points),
        kind: cur.attrs.kind,
        entity_path: entity_path.clone(),
        aggregator,
    ...
    for i in 0..points.len() {
        ...
        if p.attrs == cur.attrs {
            ...
            let cur_continuous = matches!(cur.attrs.kind, PlotSeriesKind::Continuous);
            let prev_continuous = matches!(prev_series.kind, PlotSeriesKind::Continuous);
