...
use crate::{
    ...
    visualizers::{load_keypoint_connections, process_colors, Keypoints, UiLabel, UiLabelTarget},
};
...
impl Points2DVisualizer {
    ...
    fn process_data(
        ...
        if data.instance_keys.len() <= self.max_labels {
            ...
            let colors = process_colors(data.colors.iter().cloned(), ent_path, &annotation_infos)
                .collect::<Vec<_>>();
    ...
    pub fn load_radii(
        ...
        re_tracing::profile_function!();
        let radii = crate::visualizers::process_radii_slice(radii, ent_path);
        {
