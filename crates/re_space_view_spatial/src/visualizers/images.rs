...
impl ImageVisualizer {
    ...
    fn process_image_data(
        ...
        ent_context: &SpatialSceneEntityContext<'_>,
        data: impl Iterator<Item = ImageComponentData>,
    ) {
        ...
        for data in data {
            ...
            let tensor = match ctx
                .cache
                .entry(|c: &mut TensorDecodeCache| c.entry(tensor_data_row_id, data.tensor.0))
            {
                Ok(tensor) => tensor,
                Err(err) => {
                    re_log::warn_once!(
            ...
            if let Some(textured_rect) = to_textured_rect(
                ...
                self.images.push(ViewerImage {
                    ...
                    draw_order: data.draw_order.unwrap_or(DrawOrder::DEFAULT_IMAGE),
                });
            }
        }
    }
    ...
    #[allow(clippy::too_many_arguments)]
    fn process_segmentation_image_data(
        &mut self,
        ctx: &ViewerContext<'_>,
        transforms: &TransformContext,
        ent_props: &EntityProperties,
...
impl ImageVisualizer {
    ...
    where
        F: FnMut(
            ...
            &mut dyn Iterator<Item = ImageComponentData>,
        ),
    {
        ...
        super::entity_iterator::process_archetype_uncached::<ImageVisualizer, A, _>(
            ctx,
            view_query,
            view_ctx,
            ...
            |ctx, entity_path, entity_props, spatial_ctx, results| match results {
                Results::LatestAt(_query, results) => {
                    ...
                    let tensor = match results
                        .get_dense::<TensorData>(resolver)
                        .map(|res| res.map(|mut tensors| tensors.pop()))
                    {
                        Some(Ok(Some(tensor))) => tensor,
                ...
                Results::Range(_query, results) => {
                    ...
                    let Some(tensors) = results.get_dense::<TensorData>(resolver) else {
                        return Ok(());
                    };
                    ...
                    let mut data = range_zip_1x2(tensors, draw_orders, colors).filter_map(
                        |(index, mut tensors, draw_orders, colors)| {
                            tensors.pop().map(|tensor| ImageComponentData {
                                index,
                                tensor,
                                color: colors.and_then(|mut colors| colors.pop()),
                                draw_order: draw_orders
                                    .and_then(|mut draw_orders| draw_orders.pop()),
                            })
                        },
                    );
