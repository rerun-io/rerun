...
impl VisualizerSystem for Boxes2DVisualizer {
    ...
    fn execute(
        ...
        super::entity_iterator::process_archetype::<Boxes2DVisualizer, Boxes2D, _>(
            ctx,
            view_query,
            view_ctx,
            ...
            |ctx, entity_path, _entity_props, spatial_ctx, results| {
                match results {
                    Results::LatestAt(_query, results) => {
                        ...
                        line_builder.reserve_strips(half_sizes.len() * 4)?;
                        line_builder.reserve_vertices(half_sizes.len() * 4 * 2)?;
