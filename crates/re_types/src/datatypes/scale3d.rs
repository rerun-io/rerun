            ...
            UnionArray::new(
                ...
                .clone(),
                {
                    data.iter()
                        .flatten()
                        .map(|v| match **v {
                            Scale3D::ThreeD(_) => 0i8,
                            Scale3D::Uniform(_) => 1i8,
                        })
                        .collect()
                },
                vec![
                    {
                        let (somes, three_d): (Vec<_>, Vec<_>) = data
                            .iter()
                            .flatten()
                            .filter(|datum| matches!(***datum, Scale3D::ThreeD(_)))
                            .map(|datum| {
                                let datum = match &**datum {
                                    Scale3D::ThreeD(v) => Some(v.clone()),
                                    _ => None,
                                };
                                (datum.is_some(), datum)
                            })
                    ...
                    },
                    {
                        let (somes, uniform): (Vec<_>, Vec<_>) = data
                            .iter()
                            .flatten()
                            .filter(|datum| matches!(***datum, Scale3D::Uniform(_)))
                            .map(|datum| {
                                let datum = match &**datum {
                                    Scale3D::Uniform(v) => Some(v.clone()),
                                    _ => None,
                                };
                                (datum.is_some(), datum)
                            })
