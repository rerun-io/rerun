...
fn main() -> anyhow::Result<()> {
    ...
    let all_labels = results
        .get_optional(MyLabel::name())
        .to_sparse::<MyLabel>(&resolver);

    let all_frames = range_zip_1x2(
        all_points.indexed(query.range()),
        all_colors.indexed(query.range()),
        all_labels.indexed(query.range()),
    );
