...
impl crate::ToArchetype<re_types::archetypes::Points3D> for CachedLatestAtResults {
    ...
    fn to_archetype(
    ...
    ) -> PromiseResult<re_types::archetypes::Points3D> {
        ...
        let positions = match positions.to_dense::<Position3D>(resolver).flatten() {
        ...
        let keypoint_ids = if let Some(keypoint_ids) = self.get(<KeypointId>::name()) {
            match keypoint_ids.to_dense::<KeypointId>(resolver).flatten() {
                PromiseResult::Ready(data) => Some(data.to_vec()),
                PromiseResult::Pending => return PromiseResult::Pending,
                PromiseResult::Error(err) => return PromiseResult::Error(err),
            }
