...
impl crate::ToArchetype<re_types::archetypes::Boxes3D> for CachedLatestAtResults {
    ...
    fn to_archetype(
    ...
    ) -> PromiseResult<re_types::archetypes::Boxes3D> {
        ...
        let half_sizes = match half_sizes.to_dense::<HalfSizes3D>(resolver).flatten() {
        ...
        let centers = if let Some(centers) = self.get(<Position3D>::name()) {
            match centers.to_dense::<Position3D>(resolver).flatten() {
        ...
        let rotations = if let Some(rotations) = self.get(<Rotation3D>::name()) {
            match rotations.to_dense::<Rotation3D>(resolver).flatten() {
                PromiseResult::Ready(data) => Some(data.to_vec()),
                PromiseResult::Pending => return PromiseResult::Pending,
                PromiseResult::Error(err) => return PromiseResult::Error(err),
            }
