...
pub struct Caches {
    ...
    per_cache_key: RwLock<HashMap<CacheKey, Arc<RwLock<CachesPerComponent>>>>,
}
...
impl std::fmt::Debug for Caches {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        for (cache_key, caches_per_component) in &per_cache_key {
            let caches_per_component = caches_per_component.read();
            strings.push(format!(
                "  [{cache_key:?} (pending_timeful={:?} pending_timeless={:?})]",
                caches_per_component
                    .pending_timeful_invalidation
                    .map(|t| cache_key
                        .timeline
                        .format_time_range_utc(&TimeRange::new(t, TimeInt::MAX))),
                caches_per_component.pending_timeless_invalidation,
            ));
            strings.push(indent::indent_all_by(
                4,
                format!("{caches_per_component:?}"),
            ));
        }
...
impl std::ops::Deref for Caches {
    type Target = RwLock<HashMap<CacheKey, Arc<RwLock<CachesPerComponent>>>>;
...
pub struct CachesPerComponent {
    ...
    // than an `ComponentName`: the query system doesn't care about archetypes.
    pub(crate) latest_at_per_component: RwLock<HashMap<ComponentName, Arc<RwLock<LatestAtCache>>>>,
    ...
    // TODO(cmc): At some point we should probably just store the PoV and optional components rather
    // than an `ComponentName`: the query system doesn't care about archetypes.
    pub(crate) range_per_component: RwLock<HashMap<ComponentName, Arc<RwLock<RangeCache>>>>,
...
impl std::fmt::Debug for CachesPerComponent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        let CachesPerComponent {
            latest_at_per_component,
            range_per_component,
        ...
        {
            let latest_at_per_component = latest_at_per_component.read();
            let latest_at_per_component: BTreeMap<_, _> = latest_at_per_component.iter().collect();

            for (component_name, latest_at_cache) in &latest_at_per_component {
                let latest_at_cache = latest_at_cache.read();
                strings.push(format!(
                    "[latest_at for {component_name} ({})]",
        ...
        {
            let range_per_component = range_per_component.read();
            let range_per_component: BTreeMap<_, _> = range_per_component.iter().collect();

            for (component_name, range_cache) in &range_per_component {
                let range_cache = range_cache.read();
                strings.push(format!(
                    "[range for {component_name} ({})]",
...
impl Caches {
    ...
    #[inline]
    pub fn with_latest_at<F1, F2, R1, R2>(
        &self,
        store: &DataStore,
        ...
        F2: FnMut(&LatestAtCache) -> R2,
    {
        ...
        let cache = {
            let caches_per_component = Arc::clone(self.write().entry(key.clone()).or_default());
            // Implicitly releasing top-level cache mappings -- concurrent queries can run once again.

            let removed_bytes = caches_per_component.write().handle_pending_invalidation();
            ...
            let caches_per_component = caches_per_component.read();
            let mut latest_at_per_component = caches_per_component.latest_at_per_component.write();
            Arc::clone(latest_at_per_component.entry(component_name).or_default())
        ...
        F2: FnMut(&RangeCache) -> R2,
    {
        ...
        let cache = {
            ...
            let caches_per_component = caches_per_component.read();
            let mut range_per_component = caches_per_component.range_per_component.write();
            Arc::clone(range_per_component.entry(component_name).or_default())
            // Implicitly releasing bottom-level cache mappings -- identical concurrent queries
            // can run once again.
        };
        ...
        for event in events {
            ...
            // have metrics showing that show we need to.
            {
                ...
                for entity_path in compacted.timeless {
                    for (key, caches_per_component) in caches.iter() {
                        if key.entity_path == entity_path {
                            caches_per_component.write().pending_timeless_invalidation = true;
                        }
                    }
                }
            }

            {
                ...
                for (key, time) in compacted.timeful {
                    if let Some(caches_per_component) = caches.get(&key) {
                        ...
                        let mut caches_per_component = caches_per_component.write();
                        if let Some(min_time) =
                            caches_per_component.pending_timeful_invalidation.as_mut()
                        {
                            *min_time = TimeInt::min(*min_time, time);
                        } else {
                            caches_per_component.pending_timeful_invalidation = Some(time);
                        }
                    }
                }
            }
...
impl CachesPerComponent {
    ...
    fn handle_pending_invalidation(&mut self) -> u64 {
        ...
        if pending_timeless_invalidation {
            ...
            *self = CachesPerComponent::default();
        ...
        for latest_at_cache in self.latest_at_per_component.read().values() {
        ...
        for range_cache in self.range_per_component.read().values() {
...
pub struct CacheBucket {
    ...
    pub(crate) data: Box<dyn ErasedFlatVecDeque + Send + Sync>,
...
impl CacheBucket {
    pub fn new<C: Component + Send + Sync>() -> Self {
        Self {
            component_name: C::name(),
            data_times: VecDeque::new(),
            data: Box::new(FlatVecDeque::<C>::new()) as _,
            total_size_bytes: 0,
        }
    }
}
...
impl std::fmt::Debug for CacheBucket {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        f.write_fmt(format_args!(
            "{} {component_name} values spread across {} entries ({})",
            data.dyn_num_values(),
...
impl CacheBucket {
    ...
    fn sanity_check(&self) {
        if cfg!(debug_assertions) {
            ...
            assert_eq!(self.data.dyn_num_entries(), n, "{component_name}");
        }
    }
    ...
    #[inline]
    pub fn iter_data<C: Component + Send + Sync + 'static>(
        &self,
    ) -> Option<impl Iterator<Item = &[C]>> {
        let data = self.data.as_any().downcast_ref::<FlatVecDeque<C>>()?;
        Some(data.iter())
    }
    ...
    #[inline]
    pub fn data<C: Component + Send + Sync + 'static>(&self) -> Option<&FlatVecDeque<C>> {
        self.data.as_any().downcast_ref::<FlatVecDeque<C>>()
    }

    /// Range over the batches of the specified non-optional component.
    #[inline]
    pub fn range_data<C: Component + Send + Sync + 'static>(
        &self,
        entry_range: Range<usize>,
    ) -> Option<impl Iterator<Item = &[C]>> {
        let data = self.data.as_any().downcast_ref::<FlatVecDeque<C>>()?;
    ...
    #[inline]
    pub fn data_opt<C: Component + Send + Sync + 'static>(
        &self,
    ) -> Option<&FlatVecDeque<Option<C>>> {
        self.data.as_any().downcast_ref::<FlatVecDeque<Option<C>>>()
    }

    /// Range over the batches of the specified optional component.
    #[inline]
    pub fn range_data_opt<C: Component + Send + Sync + 'static>(
        &self,
        entry_range: Range<usize>,
    ) -> Option<impl Iterator<Item = &[Option<C>]>> {
        let data = self
