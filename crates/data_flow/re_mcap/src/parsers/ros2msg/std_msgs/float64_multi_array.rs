...
/// Plugin that parses `std_msgs/msg/Float64MultiArray` messages.
pub struct Float64MultiArrayMessageParser {
    /// The array data from `Float64MultiArray` messages.
    arrays: ListBuilder<Float64Builder>,
}

impl Ros2MessageParser for Float64MultiArrayMessageParser {
...
impl MessageParser for Float64MultiArrayMessageParser {
    fn append(&mut self, _ctx: &mut ParserContext, msg: &mcap::Message<'_>) -> anyhow::Result<()> {
        let std_msgs::Float64MultiArrayMessage { data, .. } =
            cdr::try_decode_message::<std_msgs::Float64MultiArrayMessage>(&msg.data)?;
