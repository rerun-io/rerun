...
impl Telemetry {
    ...
    pub fn init(args: TelemetryArgs, drop_behavior: TelemetryDropBehavior) -> anyhow::Result<Self> {
        ...
        //
        // * If `OTEL_EXPORTER_OTLP_LOGS_ENABLED=true`, all logs will also be forwarded to the OpenTelemetry
        //   collector.
        ...
        if tracy_enabled {
            ...
            {
                tracing::warn!(
                    "using tracy in addition to standard telemetry stack, consider `OTEL_SDK_DISABLED=true`"
                );
...
// =================
//
// * Use `TRACY_ENABLED=true` combination with 'tracy' feature flag.
