...
fn plan_queues(
    ...
    let families: Vec<Family> = unsafe {
        ...
        let mut properties: Vec<vk::QueueFamilyProperties2<'_>> =
            std::iter::zip(&mut video_properties, &mut query_properties)
                .map(|(video, query)| {
        ...
        itertools::izip!(flags_and_counts, &video_properties, &query_properties)
