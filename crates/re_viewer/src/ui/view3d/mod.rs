...
use glam::Affine3A;
use macaw::{vec3, IsoTransform, Quat, Ray3, Vec3};
...
impl SpaceCamera {
    ...
    pub fn world_from_image(&self) -> Option<glam::Affine3A> {
        let intrinsics = self.intrinsics?;
        let world_from_view = self.world_from_view()?;

        let intrinsics_matrix = glam::Mat3::from_cols_array_2d(&intrinsics.intrinsics_matrix);
        Some(
            world_from_view
    ...
    pub fn image_from_world(&self) -> Option<glam::Affine3A> {
        let intrinsics = self.intrinsics?;
    ...
    pub fn unproject_as_ray(&self, pos2d: glam::Vec2) -> Option<Ray3> {
