...
use re_data_ui::{image_meaning_for_entity, item_ui, visible_history::visible_history_ui, DataUi};
...
fn entity_props_ui(
    ...
    // Note: other space view types (e.g. timeseries) handle this at the space view level.
    if space_view_class_name == SpatialSpaceView3D::NAME
        || space_view_class_name == SpatialSpaceView2D::NAME
    {
        visible_history_ui(ctx, ui, entity_path, &mut entity_props.visible_history);
    }
