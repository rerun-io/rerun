...
macro_rules! impl_query_archetype_range_xxx {
    (for N=$N:expr, M=$M:expr => povs=[$($pov:ident)+] comps=[$($comp:ident)*]) => { paste! {
        #[doc = "Cached implementation of [`re_query::query_archetype`] and [`re_query::range_archetype`]"]
        #[doc = "(combined) for `" $N "` point-of-view components and `" $M "` optional components."]
        #[allow(non_snake_case)]
        pub fn [<query_archetype_range_xxx_pov$N _comp$M>]<'a, A, $($pov,)+ $($comp,)* F>(
            &self,
            store: &'a DataStore,
            query: &RangeQuery,
            entity_path: &'a EntityPath,
        ...
        where
            ...
            $($pov: Component + Send + Sync + 'static,)+
            $($comp: Component + Send + Sync + 'static,)*
            // I1: Iterator<Item = (Option<TimeInt>, RowId)>,
            // TODO: why the tuple btw?
            F: FnMut(
                std::ops::Range<usize>,
                (
    ...
    (for N=1, M=$M:expr) => {
        seq!(COMP in 1..=$M {
            impl_query_archetype_range_xxx!(for N=1, M=$M => povs=[R1] comps=[#(C~COMP)*]);
        });
    };
}
...
impl Caches {
    seq!(NUM_COMP in 0..10 {
        impl_query_archetype_range_xxx!(for N=1, M=NUM_COMP);
    });
}
