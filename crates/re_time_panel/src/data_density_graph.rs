...
use re_log_types::{AbsoluteTimeRange, ComponentPath, TimeReal};
...
pub fn data_density_graph_ui(
    ...
    let mut hovered_time_range = AbsoluteTimeRange::EMPTY;

    {
        let mut add_data_point = |time_range: AbsoluteTimeRange, count: usize| {
            if count == 0 {
                return;
            }
        ...
        for (time_range, num_messages_at_time) in ranges {
            add_data_point(
                AbsoluteTimeRange::new(time_range.min, time_range.max),
