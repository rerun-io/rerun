...
fn simple_range() -> anyhow::Result<()> {
    ...
    let expected_points = &[
        ((TimeInt::new_temporal(323), row_id3), points3.clone()), //
    ];
    let expected_colors = &[
        ((TimeInt::new_temporal(223), row_id2), colors2.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
    ...
    let expected_points = &[
        ((TimeInt::new_temporal(123), row_id1_1), points1_1.clone()), //
        ((TimeInt::new_temporal(323), row_id3), points3.clone()),     //
    ];
    let expected_colors = &[
        ((TimeInt::new_temporal(123), row_id1_2), colors1_2.clone()), //
        ((TimeInt::new_temporal(223), row_id2), colors2.clone()),     //
    ];
    query_and_compare(
        &caches,
...
fn static_range() -> anyhow::Result<()> {
    ...
    let expected_colors = &[
        ((TimeInt::STATIC, row_id2_2), colors2_1.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
...
fn invalidation() {
    ...
    let test_invalidation = |query: RangeQuery,
        ...
        let expected_points = &[
            ((present_timestamp, row_id1), points1.clone()), //
        ];
        let expected_colors = &[
            ((present_timestamp, row_id2), colors2.clone()), //
        ];
        query_and_compare(
            &caches,
            &store,
        ...
        let expected_points = &[
            ((present_timestamp, row_id1), points1.clone()), //
            ((present_timestamp, row_id3), points3.clone()), //
        ];
        let expected_colors = &[
            ((present_timestamp, row_id2), colors2.clone()), //
        ];
        ...
        let expected_colors = &[
            ((present_timestamp, row_id2), colors2.clone()), //
            ((present_timestamp, row_id4), colors4.clone()), //
        ];
        query_and_compare(
            &caches,
            &store,
        ...
        let expected_points1 = &[
            ((past_timestamp, row_id5), points5.clone()), //
        ] as &[_];
        ...
        let expected_colors1 = &[
            ((past_timestamp, row_id6), colors6.clone()), //
        ] as &[_];
        ...
        let expected_points2 = &[
            ...
            ((present_timestamp, row_id1), points1.clone()), //
            ((present_timestamp, row_id3), points3.clone()), //
            ((future_timestamp, row_id7), points7.clone()),  //
        ] as &[_];
        let expected_points = if past_data_timepoint.is_static() {
            expected_points1
        } else {
        ...
        let expected_colors2 = &[
            ...
            ((present_timestamp, row_id2), colors2.clone()), //
            ((present_timestamp, row_id4), colors4.clone()), //
            ((future_timestamp, row_id8), colors8.clone()),  //
        ] as &[_];
        let expected_colors = if past_data_timepoint.is_static() {
            expected_colors1
        } else {
...
fn invalidation_of_future_optionals() {
    ...
    let expected_points = &[
        ((TimeInt::STATIC, row_id1), points1.clone()), //
    ];
    let expected_colors = &[];
    query_and_compare(
        &caches,
    ...
    let expected_colors = &[
        ((TimeInt::new_temporal(2), row_id2), colors2.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
    ...
    let expected_colors = &[
        ((TimeInt::new_temporal(2), row_id2), colors2.clone()), //
        ((TimeInt::new_temporal(3), row_id3), colors3.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
    ...
    let expected_colors = &[
        ((TimeInt::new_temporal(2), row_id2), colors2.clone()), //
        ((TimeInt::new_temporal(3), row_id3), colors3.clone()), //
        ((TimeInt::new_temporal(3), row_id4), colors4.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
...
fn invalidation_static() {
    ...
    let expected_colors = &[
        ((TimeInt::STATIC, row_id2), colors2.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
    ...
    let expected_colors = &[
        ((TimeInt::STATIC, row_id3), colors3.clone()), //
    ];
    query_and_compare(
        &caches,
        &store,
...
fn query_and_compare(
    ...
    expected_all_points_indexed: &[((TimeInt, RowId), Vec<MyPoint>)],
    expected_all_colors_indexed: &[((TimeInt, RowId), Vec<MyColor>)],
) {
    re_log::setup_logging();

    for _ in 0..3 {
        let cached = caches.range2(
            store,
            query,
            entity_path,
            MyPoints::all_components().iter().copied(),
        ...
        let cached_all_points_indexed = cached.iter_indexed().collect_vec();
        let cached_all_colors_indexed = cached.iter_indexed().collect_vec();

        eprintln!("{query:?}");
        eprintln!("{store}");

        similar_asserts::assert_eq!(expected_all_points_indexed, cached_all_points_indexed);
        similar_asserts::assert_eq!(expected_all_colors_indexed, cached_all_colors_indexed);
    }
}
