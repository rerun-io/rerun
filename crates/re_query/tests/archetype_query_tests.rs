...
fn simple_query() {
    ...
    let row =
        DataRow::from_cells1_sized_or_panic(RowId::random(), ent_path, timepoint, 2, positions);
...
fn no_instance_join_query() {
    ...
    let row = DataRow::from_cells1_sized_or_panic(RowId::random(), ent_path, timepoint, 2, colors);
    store.insert_row(&row).unwrap();
