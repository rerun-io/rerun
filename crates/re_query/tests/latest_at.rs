...
use re_data_store2::{external::re_chunk::Chunk, DataStore2, LatestAtQuery, StoreSubscriber2 as _};
use re_log_types::{
    build_frame_nr,
    example_components::{MyColor, MyPoint, MyPoints},
    EntityPath, RowId, TimeInt, TimePoint,
};
use re_query::Caches;
...
fn simple_query() {
    ...
    let query = re_data_store2::LatestAtQuery::new(timepoint[0].0, timepoint[0].1);
...
//     insert_and_react(&mut store, &mut caches, &row1);
//
//     let query = re_data_store2::LatestAtQuery::new(query_time[0].0, query_time[0].1);
