...
use re_query::{range_zip_1x1, Caches};
...
fn query_and_compare(
    ...
    expected_colors: Option<&[MyColor]>,
) {
    re_log::setup_logging();

    for _ in 0..3 {
        let cached = caches.latest_at2(
            store,
            query,
            entity_path,
        ...
        eprintln!("{entity_path} @ {query:?}");
        eprintln!("{store}");

        let (points_index, points) = cached.iter_indexed::<MyPoint>().next().unwrap();
        similar_asserts::assert_eq!(expected_compound_index, points_index);
        similar_asserts::assert_eq!(expected_points, points);

        if let Some(expected_colors) = expected_colors {
            let (colors_index, colors) = cached.iter_indexed::<MyColor>().next().unwrap();
            similar_asserts::assert_eq!(expected_compound_index, colors_index);
            similar_asserts::assert_eq!(expected_colors, colors);
        }
    }
}
