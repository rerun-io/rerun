...
impl<A: Archetype> ArchetypeView<A> {
    ...
    pub fn iter_optional_component<'a, C: Component + Clone + 'a>(
        ...
        if let Some(component) = component {
            ...
            let primary_instance_key_buffer = self.required_comp().instance_keys.as_arrow_ref();
            let component_instance_key_buffer = component.instance_keys.as_arrow_ref();
            if primary_instance_key_buffer == component_instance_key_buffer {
