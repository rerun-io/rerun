...
pub enum DecoderState {
    ...
    WaitingForMessagePayload(crate::rrd::MessageHeader),
                ...
                if let Some(bytes) = self.byte_chunks.try_read(header.len as usize) {
                    ...
                    if let Some(message) = message {
                        ...
                        if !bytes.is_empty() {
                            ...
                            // Backward compatibility: if the payload is empty (i.e. `header.len == 0`), that can
                            // only mean that the `End` message was written by a legacy encoder that predates the
                            // introduction of footers.
                            // In that case, we consider the current stream to be over, and immediately start looking
                            // for the beginning of a new one (in case of concatenated streams), if any.
                            // self.state = DecoderState::WaitingForStreamHeader;

                            // TODO: this is not only because of BW, it's also the case when the recording
                            // doesn't have any actual arrowmsgs (e.g. like the tests below that
                            // only include SetStoreInfo messages).
                            self.state = DecoderState::WaitingForStreamFooter;
                        }
