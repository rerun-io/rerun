...
// TODO: we really need to document the rare scenarios where one can see more than one footer
// (which AFAIK are the same ones as multiple headers, so prob should document that too).
// which is basically just non-semantically concatenated streams (e.g. cat *.rrd | rerun ...).
...
pub struct StreamFooter {
    ...
    //
    // TODO: i know this is a temp protocol, but i still think using the same fourcc is needlessly
    // risky.
    pub fourcc: [u8; 4],

    // TODO: let's introduce identifiers then
    // -> 'FOOT'
    // pub identifier: [u8; 4],
    //
    //
    /// The position in bytes where the serialized `RrdManifest` payload starts, excluding the
    /// message header.
    ///
    /// I.e. a transport-level `RrdManifest` can be decoded from the bytes at (pseudo-code):
    /// ```text
    /// let start = footer.rrd_manifest_byte_offset_from_start_excluding_header;
    /// let end = start + footer.rrd_manifest_byte_size_excluding_header;
    /// let bytes = &file[start..end];
    /// let rrd_manifest = re_protos::RrdManifest::decode(bytes)?;
    /// let rrd_manifest = rrd_manifest.to_application()?;
    /// ```
    pub rrd_manifest_byte_offset_from_start_excluding_header: u64,

    /// The size in bytes of the serialized `RrdManifest` payload, excluding the message header.
    ///
    ...
    pub rrd_manifest_byte_size_excluding_header: u64,

    /// Checksum for the RRD manifest payload.
    ///
...
impl StreamFooter {
    pub const ENCODED_SIZE_BYTES: usize = 24;
    pub const CRC_SEED: u32 = 7850921; // "RERUN" in base 26 (A=0, Z=25)

    pub fn new(
        rrd_manifest_byte_offset_from_start_excluding_header: u64,
        rrd_manifest_byte_size_excluding_header: u64,
    ...
    pub fn from_rrd_manifest_bytes(
        rrd_manifest_byte_offset_from_start_excluding_header: u64,
        rrd_manifest_bytes: &[u8],
    ) -> Self {
        let crc_excluding_header = xxhash_rust::xxh32::xxh32(rrd_manifest_bytes, Self::CRC_SEED);
        Self {
            fourcc: crate::RRD_FOURCC,
            rrd_manifest_byte_offset_from_start_excluding_header,
            rrd_manifest_byte_size_excluding_header: rrd_manifest_bytes.len() as u64,
            crc_excluding_header,
        }
    }
}
...
impl Encodable for StreamFooter {
    fn to_rrd_bytes(&self, out: &mut Vec<u8>) -> Result<u64, crate::rrd::CodecError> {
        ...
        out.extend_from_slice(fourcc);
        out.extend_from_slice(&rrd_manifest_byte_offset_from_start_excluding_header.to_le_bytes());
        out.extend_from_slice(&rrd_manifest_byte_size_excluding_header.to_le_bytes());
...
impl Decodable for StreamFooter {
    fn from_rrd_bytes(data: &[u8]) -> Result<Self, crate::rrd::CodecError> {
        ...
        let rrd_manifest_byte_offset_from_start_excluding_header =
            u64::from_le_bytes(data[4..12].try_into().expect("cannot fail, checked above"));
        let rrd_manifest_byte_size_excluding_header =
            u64::from_le_bytes(data[12..20].try_into().expect("cannot fail, checked above"));
        let crc = u32::from_le_bytes(data[20..24].try_into().expect("cannot fail, checked above"));
