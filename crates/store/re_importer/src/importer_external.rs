...
///
/// External importers are _not_ registered on a per-extension basis: we want users to be able to
/// filter data on a much more fine-grained basis than just file extensions (e.g. checking the file
/// itself for magic bytes).
pub static EXTERNAL_IMPORTER_PATHS: LazyLock<Vec<PathBuf>> = LazyLock::new(|| {
    re_tracing::profile_scope!("initialize-external-importers");
    ...
    for dirpath in dirpaths {
        ...
        let paths = dir.into_iter().filter_map(|entry| {
            ...
            let is_new_prefix = filename_str.starts_with(EXTERNAL_IMPORTER_PREFIX);
            let is_legacy_prefix = filename_str.starts_with(EXTERNAL_IMPORTER_PREFIX_DEPRECATED);
    ...
    executables
        ...
        .filter_map(|(name, paths)| {
            if paths.len() > 1 {
                re_log::debug!(name, ?paths, "Found duplicated external importer in $PATH");
            }
...
/// Iterator over all registered external [`crate::Importer`]s.
#[inline]
pub fn iter_external_importers() -> impl ExactSizeIterator<Item = PathBuf> {
    re_tracing::profile_wait!("EXTERNAL_IMPORTER_PATHS");
    EXTERNAL_IMPORTER_PATHS.iter().cloned()
}
...
impl crate::Importer for ExternalImporter {
    ...
    fn import_from_path(
        ...
        let external_importers = {
            re_tracing::profile_wait!("EXTERNAL_IMPORTER_PATHS");
            EXTERNAL_IMPORTER_PATHS.iter()
        };

        #[derive(Debug, PartialEq, Eq)]
        struct CompatibleImporterFound;
        let (tx_feedback, rx_feedback) = crossbeam::channel::bounded::<CompatibleImporterFound>(64);

        let args = settings.to_cli_args();
        for exe in external_importers {
            let args = args.clone();
            ...
            _ = std::thread::Builder::new().name(exe.to_string_lossy().to_string()).spawn(move || {
                ...
                re_log::debug!(?filepath, importer = ?exe, "Importing data from filesystem using external importer…",);
                ...
                // We have to wait in order to know whether the child process is a compatible importer.
                //
                    ...
                    match child.try_wait() {
                        ...
                        Ok(None) => {
                            if is_sending_data.load(std::sync::atomic::Ordering::Relaxed) {
                                ...
                                // Let's get out ASAP.
                                re_log::debug!(importer = ?exe, ?filepath, "compatible external importer found");
                ...
                let is_compatible =
                    status.code() != Some(crate::EXTERNAL_IMPORTER_INCOMPATIBLE_EXIT_CODE);
                ...
                if is_compatible {
                    ...
                    if status.success() {
                        ...
                        // Include any log output of the external importer in the console, because maybe it has useful information:
                        let stderr_indented = stderr_str.lines().map(|line| format!("  {line}")).collect::<Vec<_>>().join("\n");
                        re_log::debug!("External importer stderr:\n{stderr_indented}");
        ...
        let any_compatible_importer = rx_feedback.recv() == Ok(CompatibleImporterFound);
        if !any_compatible_importer {
            // NOTE: The only way to get here is if all importers closed the sending end of the
...
fn decode_and_stream(
    ...
    for msg in msgs {
        ...
        let data = ImportedData::LogMsg(ExternalImporter::name(&ExternalImporter), msg);
