...
It also covers the two compression filters h5py offers: `gzip` (`qvel`) and `lzf` (`qacc`).
...
with h5py.File(out, "w") as f:
    ...
    obs.create_dataset("qacc", data=np.arange(4, dtype=np.float32) * 10.0, compression="lzf")
```
