...
impl TimeHistogramPerTimeline {
    ...
    pub fn on_rrd_manifest(&mut self, rrd_manifest_index: &RrdManifestIndex) {
        ...
        for chunk in rrd_manifest_index.virtual_chunks() {
        ...
        for event in events {
            match &event.diff {
                ChunkStoreDiff::Addition(add) => {
                    ...
                    let virtual_chunk_id = add.chunk_before_processing.id();
                    let virtual_chunk_info =
                        rrd_manifest_index.virtual_chunk_info(&virtual_chunk_id);
                    ...
                    if delta_chunk.is_static() {
                        ...
                        for time_column in delta_chunk.timelines().values() {
                            ...
                            if let Some(chunk_info) = virtual_chunk_info
                ...
                ChunkStoreDiff::Deletion(del) => {
                    if del.chunk.is_static() {
                        ...
                        let virtual_chunk_ids = store
                            .find_root_rrd_manifests(&del.chunk.id())
                            .into_iter()
                            .map(|(c, _)| c)
                            .collect_vec();
                        let virtual_chunk_infos = virtual_chunk_ids
                            .iter()
                            .filter_map(|cid| rrd_manifest_index.virtual_chunk_info(cid))
                            .collect_vec();
                        ...
                        for time_column in del.chunk.timelines().values() {
                            ...
                            for chunk_info in &virtual_chunk_infos {
