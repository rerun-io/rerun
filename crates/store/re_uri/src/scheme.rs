...
/// `rerun` and `rerun+https` parses to the same thing, but we prefer to display just `rerun`.
#[derive(
...
impl std::str::FromStr for Scheme {
    ...
    fn from_str(url: &str) -> Result<Self, Self::Err> {
        if url.starts_with("rerun+http://") {
            Ok(Self::RerunHttp)
        } else if url.starts_with("rerun://") || url.starts_with("rerun+https://") {
            Ok(Self::RerunHttps)
        } else {
