...
/// Keep in mind that there 3 different layers of `LogMsg`:
/// * There is the native application-level type (`re_log_types::LogMsg`), which is used everywhere in
///    the OSS application code, and performs all kinds of migrations and data transformations.
///    This isn't used in Redap, where everything is done at the transport-level always.
/// * There is the transport-level type _with_ oneof metadata (`re_protos::log_msg::v1alpha1::LogMsg`),
///    which is used exclusively in the legacy gRPC API (aka SDK comms) and _nowhere else_.
/// * Finally, there is the transport-level type _without_ oneof metadata (`re_protos::log_msg::v1alpha1::log_msg::Msg`),
///    which is used exclusively by the RRD protocol, where oneof metadata are replaced by message headers.
///
