...
use re_types_core::{archetypes::Clear, ComponentDescriptor};
...
pub struct QueryCacheKey {
    ...
    pub component_desc: ComponentDescriptor,
}
...
impl re_byte_size::SizeBytes for QueryCacheKey {
    ...
    fn heap_size_bytes(&self) -> u64 {
        ...
        entity_path.heap_size_bytes()
            ...
            + component_desc.heap_size_bytes()
    }
}
...
impl QueryCacheKey {
    ...
    pub fn new(
        ...
        component_desc: impl Into<ComponentDescriptor>,
    ) -> Self {
        Self {
            entity_path: entity_path.into(),
            timeline: timeline.into(),
            component_desc: component_desc.into(),
        }
    }
}
...
impl ChunkStoreSubscriber for QueryCache {
    ...
    fn on_events(&mut self, events: &[ChunkStoreEvent]) {
                ...
                if chunk.is_static() {
                    for component_desc in chunk.component_descriptors() {
                        let compacted_events = compacted_events
                            .static_
                            .entry((chunk.entity_path().clone(), component_desc))
                            .or_default();
...

        {
            ...
            for ((entity_path, component_desc), chunk_ids) in compacted_events.static_ {
                ...
                for (key, cache) in caches_latest_at.iter() {
                    if key.entity_path == entity_path && key.component_desc == component_desc {
...

        {
            ...
            for (key, time) in compacted_events.temporal_latest_at {
                if key.component_desc == Clear::descriptor_is_recursive() {
