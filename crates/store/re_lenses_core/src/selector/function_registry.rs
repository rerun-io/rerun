...
pub struct FunctionRegistry {
    constructors: ahash::HashMap<String, FunctionConstructor>,
}
...
impl re_byte_size::SizeBytes for FunctionRegistry {
    fn heap_size_bytes(&self) -> u64 {
        ...
        constructors.capacity() as u64
            * (std::mem::size_of::<String>() + std::mem::size_of::<FunctionConstructor>()) as u64
            + constructors
                .keys()
...
impl FunctionRegistry {
    ...
    #[inline]
    pub fn register<Args, F: TransformConstructor<Args>>(
        &mut self,
...
pub trait TransformConstructor<Args>: Send + Sync + 'static {
    fn constructor(&self, arguments: &[Literal]) -> Option<BoxedFunction>;
}

macro_rules! impl_transform_constructors {
    () => {
        impl_transform_constructors!(impl);
    };
    (impl $($ident:ident)*) => {
    ...
    ($head:ident $($tail:ident)*) => {
        impl_transform_constructors!($($tail)*);
        impl_transform_constructors!(impl $head $($tail)*);
    };
}

impl_transform_constructors!(T0 T1 T2 T3 T4 T5 T6 T7 T8 T9 T10 T11 T12);
