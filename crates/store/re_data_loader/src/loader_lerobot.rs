...
use arrow::{
    array::{
        ArrayRef, BinaryArray, FixedSizeListArray, Int64Array, ListArray, RecordBatch, StringArray,
        StructArray,
    },
    compute::cast,
...
fn extract_list_array_elements_as_f64(data: &ListArray) -> anyhow::Result<Vec<ArrayRef>> {
    (0..data.len())
        .map(|idx| {
            cast(&data.value(idx), &DataType::Float64)
                .with_context(|| format!("Failed to cast {:?} to Float64", data.data_type()))
