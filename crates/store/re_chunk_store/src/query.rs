...
impl ChunkStore {
    ...
    /// If the entity has static component data associated with it, it will unconditionally
    /// override any temporal component data.
    pub fn range_relevant_chunks_larger_than_ram(
        &self,
        query: &RangeQuery,
        entity_path: &EntityPath,
        component: ComponentIdentifier,
        ...
        let (chunks, missing_chunk_ids) = self.range_larger_than_ram(
            query,
            self.temporal_chunk_ids_per_entity_per_component
                .get(entity_path)
                .and_then(|temporal_chunk_ids_per_timeline| {
    ...
    /// The caller should filter the returned chunks further (see [`Chunk::range`]) in order to
    /// determine how exactly each row of data fit with the rest.
    pub fn range_relevant_chunks_for_all_components_larger_than_ram(
        &self,
        query: &RangeQuery,
        entity_path: &EntityPath,
        include_static: bool,
        ...
        let (chunks, chunk_ids_missing) = if include_static {
            ...
            // All temporal chunks for the given entity, filtered by components
            // for which we already have static chunks.
            let (temporal_chunks, temporal_chunk_ids_missing) = self.range_larger_than_ram(
                query,
                self.temporal_chunk_ids_per_entity_per_component
                    .get(entity_path)
                    .and_then(|temporal_chunk_ids_per_timeline_per_component| {
            ...
            let (chunks, chunk_ids_missing) = self.range_larger_than_ram(
                query,
                self.temporal_chunk_ids_per_entity
                    .get(entity_path)
                    .and_then(|temporal_chunk_ids_per_timeline| {
            ...
            // This cannot yield duplicates by definition.
            (Either::Right(chunks), chunk_ids_missing)
        };
    ...
    fn range_larger_than_ram<'a>(
        ...
        let (chunks, missing_chunk_ids): (Vec<_>, Vec<_>) = temporal_chunk_ids_per_times
            .map(|temporal_chunk_ids_per_time| {
                // See `RangeQueryOptions::include_extended_bounds` for more information.
                let query_min = if query.options().include_extended_bounds {
                    re_log_types::TimeInt::new_temporal(
            ...
            .flat_map(|temporal_chunk_ids| {
                temporal_chunk_ids.iter().map(|chunk_id| {
        ...
        let chunks = chunks.into_iter().flatten().collect_vec();
        let missing_chunk_ids = missing_chunk_ids.into_iter().flatten().collect_vec();
