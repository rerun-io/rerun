...
pub struct ChunkStoreDiff {
    ...
    ///
    /// When trying to count things, use [`ChunkStoreDiff::only_new_data`], which always returns
...
impl ChunkStoreDiff {
    ...
    pub fn only_new_data(&self) -> &Chunk {
        #[expect(clippy::match_same_arms)] // the explicitness is important
        match self.direct_lineage {
            Some(ChunkDirectLineageReport::CompactedFrom(_)) => &self.chunk_before_processing,
            Some(ChunkDirectLineageReport::SplitFrom(_, _)) => &self.chunk_after_processing,
