...
fn array_slice_memory_size(array: &dyn ArrowArray) -> u64 {
...
fn list_array_slice_memory_size(list_array: &ArrowListArray) -> u64 {
...
impl re_byte_size::SizeBytes for Chunk {
    ...
    fn heap_size_bytes(&self) -> u64 {
        ...
        if size_bytes == 0 {
            // Calculate component sizes using accurate slice memory sizing
            let components_size = components
                .values()
                .map(|column| {
                    array_slice_memory_size(&column.list_array)
                        + column.descriptor.heap_size_bytes()
                })
                .sum::<u64>();
            ...
            size_bytes = id.heap_size_bytes()
                ...
                + array_slice_memory_size(row_ids)
...
impl re_byte_size::SizeBytes for TimeColumn {
    ...
    fn heap_size_bytes(&self) -> u64 {
        ...
        timeline.heap_size_bytes()
            + (times.len() * std::mem::size_of::<i64>()) as u64 // accurate slice size
            + is_sorted.heap_size_bytes()
