...
#[derive(Debug, Clone)]
pub struct ChunkSharedMono(ChunkShared);
...
impl ChunkSharedMono {
    ...
    pub fn component_instance<C: Component>(&self, instance_index: usize) -> Option<C> {
        ...
        let unit_batch = match C::from_arrow(&*array) {
            Ok(data) => Some(data),
...
impl Chunk {
    ...
    pub fn component_batch_raw(
        ...
        self.components.get(component_name).and_then(|list_array| {
            (list_array.len() > row_index).then(|| list_array.value(row_index))
        })
    }
    ...
    pub fn component_instance_raw(
        ...
        let array = self.component_batch_raw(component_name, row_index)?;
        (array.len() > instance_index).then(|| array.sliced(instance_index, 1))
    }
    ...
    pub fn component_batch<C: Component>(&self, row_index: usize) -> Option<Vec<C>> {
        ...
        match C::from_arrow(&*self.component_batch_raw(&C::name(), row_index)?) {
            Ok(data) => Some(data),
