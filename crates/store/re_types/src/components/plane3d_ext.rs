...
impl Plane3D {
    ...
    pub const YZ: Self = Plane3D(crate::datatypes::Plane3D([1.0, 0.0, 0.0, 0.0]));

    /// The Z^X plane with normal = +Y.
    pub const ZX: Self = Plane3D(crate::datatypes::Plane3D([0.0, 1.0, 0.0, 0.0]));

    /// The X^Y plane with normal = +Z.
    pub const XY: Self = Plane3D(crate::datatypes::Plane3D([0.0, 0.0, 1.0, 0.0]));
    ...
    pub fn new(normal: impl Into<crate::datatypes::Vec3D>, distance: f32) -> Self {
        Plane3D(crate::datatypes::Plane3D::new(normal, distance))
    }
}
