...
impl ViewportBlueprint {
        ...
        I: IntoIterator<Item = usize> + Clone,
    {
        let columns = [
            self.root_container
                .map(|root_container| root_container.partitioned(_lengths.clone()))
                .transpose()?,
            self.maximized
                .map(|maximized| maximized.partitioned(_lengths.clone()))
                .transpose()?,
            self.auto_layout
                .map(|auto_layout| auto_layout.partitioned(_lengths.clone()))
                .transpose()?,
            self.auto_views
                .map(|auto_views| auto_views.partitioned(_lengths.clone()))
                .transpose()?,
            self.past_viewer_recommendations
                .map(|past_viewer_recommendations| {
                    past_viewer_recommendations.partitioned(_lengths.clone())
                })
                .transpose()?,
        ];
