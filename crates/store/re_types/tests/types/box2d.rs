use re_types::{
    Archetype as _, AsComponents as _, ComponentBatch as _, archetypes::Boxes2D, components,
};
...
fn roundtrip() {
    let expected = Boxes2D {
        ...
        .serialized()
        .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_half_sizes())),
        centers: vec![
            components::Position2D::new(1.0, 2.0),
            components::Position2D::new(3.0, 4.0),
        ]
        .serialized()
        .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_centers())),
        colors: vec![
            components::Color::from_unmultiplied_rgba(0xAA, 0x00, 0x00, 0xCC),
            components::Color::from_unmultiplied_rgba(0x00, 0xBB, 0x00, 0xDD),
        ]
        .serialized()
        .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_colors())),
        radii: vec![
            components::Radius::from(42.0),
            components::Radius::from(43.0),
        ]
        .serialized()
        .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_radii())),
        labels: vec![
            components::Text::from("hello"),
            components::Text::from("friend"),
        ]
        .serialized()
        .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_labels())),
        draw_order: components::DrawOrder(300.0.into())
            .serialized()
            .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_draw_order())),
        class_ids: vec![
            components::ClassId::from(126),
            components::ClassId::from(127),
        ]
        .serialized()
        .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_class_ids())),
        show_labels: components::ShowLabels(true.into())
            .serialized()
            .map(|batch| batch.with_descriptor_override(Boxes2D::descriptor_show_labels())),
    };
