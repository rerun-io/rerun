//! Parsing of [PLY](https://en.wikipedia.org/wiki/PLY_(file_format)) point clouds.
//!
//! `.ply` is versatile enough to hold a 2D or a 3D point cloud, so both
//! [`Points2D`] and [`Points3D`] are read through the same parser here.
//! Meshes are handled by the viewer instead, via `Asset3D`.
        ...
        let vertex_unknown_props = header
            ...
            .map(|element_def| {
                element_def
                    ...
                    .keys()
                    .filter(|name| {
                        !POINT_VERTEX_PROPERTIES.contains(&name.as_str())
                            && !matches!(
                                name.as_str(),
                                PROP_SCALE_X
                                    | PROP_SCALE_Y
