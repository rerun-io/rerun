...
impl<E: StorageEngineLike> QueryHandle<E> {
    ...
    fn fetch_clear_chunks(
        ...
        fn entity_path_ancestors(entity_path: &EntityPath) -> impl Iterator<Item = EntityPath> + use<> {
            std::iter::from_fn({
    ...
    pub fn next_row_async(&self) -> impl std::future::Future<Output = Option<Vec<ArrayRef>>> + use<E>
    where
        E: 'static + Send + Clone,
    {
