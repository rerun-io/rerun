// DO NOT EDIT! This file was auto-generated by crates/build/re_types_builder/src/codegen/rust/api.rs
// Based on "crates/build/re_type_definitions/rerun/encodings/uint32.def.rs".

#![allow(unused_braces)]
#![allow(unused_imports)]
#![allow(unused_parens)]
#![allow(clippy::allow_attributes)]
#![allow(clippy::clone_on_copy)]
#![allow(clippy::cloned_instead_of_copied)]
#![allow(clippy::eq_op)]
#![allow(clippy::map_flatten)]
#![allow(clippy::needless_question_mark)]
#![allow(clippy::new_without_default)]
#![allow(clippy::redundant_closure)]
#![allow(clippy::too_many_arguments)]
#![allow(clippy::too_many_lines)]
#![allow(clippy::wildcard_imports)]

use crate::SerializationResult;
use crate::SerializedComponentBatch;
use crate::try_serialize_field;
use crate::{ComponentDescriptor, ComponentType};
use crate::{DeserializationError, DeserializationResult};
use ::arrow::array::ArrayRef;
use ::std::borrow::Cow;

/// **Encoding**: A 32bit unsigned integer.
#[derive(
    Clone, Debug, Default, Copy, PartialEq, Eq, PartialOrd, Ord, ::re_byte_size::SizeBytes,
)]
pub struct UInt32(pub u32);

crate::macros::impl_into_cow!(UInt32);

impl crate::ArrowDatatype for UInt32 {
    #[inline]
    fn arrow_datatype() -> arrow::datatypes::DataType {
        use arrow::datatypes::*;
        DataType::UInt32
    }
}

impl crate::ToArrow for UInt32 {
    fn to_arrow<'a>(
        data: impl IntoIterator<Item = impl Into<Cow<'a, Self>>>,
    ) -> SerializationResult<ArrayRef>
    where
        Self: Clone + 'a,
    {
        #![allow(clippy::manual_is_variant_and)]
        use crate::{
            ArrowDatatype as _, ResultExt as _, ToArrow as _, ToArrowOpt as _,
            arrow_helpers::as_array_ref,
        };
        use arrow::{array::*, buffer::*, datatypes::*};
        Ok({
            let data0: Vec<_> = data
                .into_iter()
                .map(|datum| {
                    let datum: Cow<'a, Self> = datum.into();
                    datum.into_owned().0
                })
                .collect();
            let data0_validity = None;
            as_array_ref(PrimitiveArray::<UInt32Type>::new(
                data0.into_iter().collect(),
                data0_validity,
            ))
        })
    }
}

impl crate::FromArrow for UInt32 {
    #[inline]
    fn from_arrow(arrow_data: &dyn arrow::array::Array) -> DeserializationResult<Vec<Self>> {
        use crate::{
            ArrowDatatype as _, FromArrow as _, FromArrowOpt as _, ResultExt as _,
            arrow_helpers::*, arrow_zip_validity::ZipValidity,
        };
        use arrow::{array::*, buffer::*, datatypes::*};
        err_on_nulls(arrow_data, "rerun.encodings.UInt32")?;
        Ok({
            let slice = arrow_data
                .try_cast::<UInt32Array>(|| DataType::UInt32)
                .with_context("rerun.encodings.UInt32#value")?
                .values()
                .as_ref();
            { slice.iter().copied().map(Self).collect::<Vec<_>>() }
        })
    }
}

impl From<u32> for UInt32 {
    #[inline]
    fn from(value: u32) -> Self {
        Self(value)
    }
}

impl From<UInt32> for u32 {
    #[inline]
    fn from(value: UInt32) -> Self {
        value.0
    }
}
