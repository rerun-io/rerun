use crate::{
    ComponentBatch, ComponentBatchCowWithDescriptor, LoggableBatch as _, SerializationResult,
...
pub trait AsComponents {
    ...
    fn as_component_batches_v2(&self) -> Vec<SerializedComponentBatch> {
        self.as_component_batches()
            .into_iter()
            .map(|batch| SerializedComponentBatch {
                array: batch.to_arrow().unwrap(),
                descriptor: batch.descriptor().into_owned(),
            })
            .collect()
    }
...
impl<const N: usize> AsComponents for [&dyn ComponentBatch; N] {
    #[inline]
    fn as_component_batches_v2(&self) -> Vec<SerializedComponentBatch> {
        self.iter().map(|batch| batch.serialized()).collect()
    }
}
