...
class TestFlowWorkflowMapping:
    ...
    def test_flow_to_workflow_name(self) -> None:
        ...
        assert github.flow_to_workflow_name("ci") == "recompose_flow_ci.yml"
        assert github.flow_to_workflow_name("build_test") == "recompose_flow_build_test.yml"

    def test_workflow_to_flow_name(self) -> None:
        """workflow_to_flow_name extracts flow name correctly."""
        assert github.workflow_to_flow_name("recompose_flow_ci.yml") == "ci"
        assert github.workflow_to_flow_name("recompose_flow_build_test.yml") == "build_test"

    def test_workflow_to_flow_name_non_recompose(self) -> None:
        """workflow_to_flow_name returns None for non-recompose workflows."""
        assert github.workflow_to_flow_name("ci.yml") is None
        assert github.workflow_to_flow_name("build.yaml") is None
...
class TestGhCliWrapper:
    ...
    @patch("recompose.github.subprocess.run")
    def test_run_gh_success(self, mock_run: MagicMock) -> None:
        ...
        result = github._run_gh("test", "command")
        assert result.ok
    ...
    def test_run_gh_json_parsing(self, mock_run: MagicMock) -> None:
        ...
        result = github._run_gh("test", capture_json=True)
        assert result.ok
...
class TestValidateWorkflowSync:
    ...
    def test_validate_sync_local_missing(self, tmp_path: Path) -> None:
        ...
        result = github.validate_workflow_sync(
    ...
    @patch("recompose.github.get_workflow_file_content")
    def test_validate_sync_remote_missing(self, mock_get: MagicMock, tmp_path: Path) -> None:
        ...
        mock_get.return_value = github.Err("Not found")

        result = github.validate_workflow_sync(local_file, ".github/workflows/test.yml")
        assert result.failed
        assert "not found" in str(result.error).lower()
    ...
    def test_validate_sync_files_match(self, mock_get: MagicMock, tmp_path: Path) -> None:
        ...
        mock_get.return_value = github.Ok(content)
    ...
    def test_validate_sync_files_differ(self, mock_get: MagicMock, tmp_path: Path) -> None:
        ...
        mock_get.return_value = github.Ok("name: Test\non: pull_request\n")
