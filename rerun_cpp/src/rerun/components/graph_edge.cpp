...
namespace rerun {
    const std::shared_ptr<arrow::DataType>& Loggable<components::GraphEdge>::arrow_datatype() {
        static const auto datatype = arrow::list(
            arrow::field("item", Loggable<rerun::datatypes::GraphEdge>::arrow_datatype(), false)
        );
        return datatype;
    }
    ...
    rerun::Error Loggable<components::GraphEdge>::fill_arrow_array_builder(
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.edge.data()) {
                RR_RETURN_NOT_OK(Loggable<rerun::datatypes::GraphEdge>::fill_arrow_array_builder(
                    value_builder,
                    element.edge.data(),
                    element.edge.size()
                ));
