...
namespace rerun::components {
    ...
    struct Opacity {
        rerun::encodings::Float32 opacity;

      public:
        Opacity() = default;

        Opacity(rerun::encodings::Float32 opacity_) : opacity(opacity_) {}

        Opacity& operator=(rerun::encodings::Float32 opacity_) {
            opacity = opacity_;
            return *this;
        }
...
namespace rerun {
    static_assert(sizeof(rerun::encodings::Float32) == sizeof(components::Opacity));

    /// \private
    template <>
    struct Loggable<components::Opacity> {
        static constexpr std::string_view ComponentType = "rerun.components.Opacity";
