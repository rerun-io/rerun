...
// Based on "crates/build/re_type_definitions/rerun/components/vector2d.def.rs".
...
namespace rerun::components {
    ...
    struct Vector2D {
      ...
      public:
        ...
        Vector2D(rerun::encodings::Vec2D vector_) : vector(vector_) {}

        Vector2D& operator=(rerun::encodings::Vec2D vector_) {
            vector = vector_;
            return *this;
        }
...
namespace rerun {
    static_assert(sizeof(rerun::encodings::Vec2D) == sizeof(components::Vector2D));

    /// \private
    template <>
    struct Loggable<components::Vector2D> {
        static constexpr std::string_view ComponentType = "rerun.components.Vector2D";
