...
// Based on "crates/build/re_type_definitions/rerun/components/plane3d.def.rs".
...
namespace rerun::components {
    ...
    struct Plane3D {
        rerun::encodings::Plane3D xyzd;

      public:
        Plane3D() = default;

        Plane3D(rerun::encodings::Plane3D xyzd_) : xyzd(xyzd_) {}

        Plane3D& operator=(rerun::encodings::Plane3D xyzd_) {
            xyzd = xyzd_;
            return *this;
        }

        /// Cast to the underlying Plane3D encoding
        operator rerun::encodings::Plane3D() const {
            return xyzd;
        }
    };
} // namespace rerun::components
...
namespace rerun {
    static_assert(sizeof(rerun::encodings::Plane3D) == sizeof(components::Plane3D));
    ...
    struct Loggable<components::Plane3D> {
        ...
        static const std::shared_ptr<arrow::DataType>& arrow_data_type() {
            return Loggable<rerun::encodings::Plane3D>::arrow_data_type();
        }
        ...
        static Result<std::shared_ptr<arrow::Array>> to_arrow(
            ...
            if (num_instances == 0) {
                return Loggable<rerun::encodings::Plane3D>::to_arrow(nullptr, 0);
                ...
                return Loggable<rerun::encodings::Plane3D>::to_arrow(
                    &instances->xyzd,
                    num_instances
                );
            }
