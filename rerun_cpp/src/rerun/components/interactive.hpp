// DO NOT EDIT! This file was auto-generated by crates/build/re_types_builder/src/codegen/cpp/mod.rs
// Based on "crates/build/re_type_definitions/rerun/components/interactive.def.rs".

#pragma once

#include "../encodings/bool.hpp"
#include "../result.hpp"

#include <cstdint>
#include <memory>

namespace rerun::components {
    /// **Component**: Whether the entity can be interacted with.
    ///
    /// Non interactive components are still visible, but mouse interactions in the view are disabled.
    struct Interactive {
        rerun::encodings::Bool interactive;

      public:
        Interactive() = default;

        Interactive(rerun::encodings::Bool interactive_) : interactive(interactive_) {}

        Interactive& operator=(rerun::encodings::Bool interactive_) {
            interactive = interactive_;
            return *this;
        }

        Interactive(bool value_) : interactive(value_) {}

        Interactive& operator=(bool value_) {
            interactive = value_;
            return *this;
        }

        /// Cast to the underlying Bool encoding
        operator rerun::encodings::Bool() const {
            return interactive;
        }
    };
} // namespace rerun::components

namespace rerun {
    static_assert(sizeof(rerun::encodings::Bool) == sizeof(components::Interactive));

    /// \private
    template <>
    struct Loggable<components::Interactive> {
        static constexpr std::string_view ComponentType = "rerun.components.Interactive";

        /// Returns the arrow data type this type corresponds to.
        static const std::shared_ptr<arrow::DataType>& arrow_datatype() {
            return Loggable<rerun::encodings::Bool>::arrow_datatype();
        }

        /// Serializes an array of `rerun::components::Interactive` into an arrow array.
        static Result<std::shared_ptr<arrow::Array>> to_arrow(
            const components::Interactive* instances, size_t num_instances
        ) {
            if (num_instances == 0) {
                return Loggable<rerun::encodings::Bool>::to_arrow(nullptr, 0);
            } else if (instances == nullptr) {
                return rerun::Error(
                    ErrorCode::UnexpectedNullArgument,
                    "Passed array instances is null when num_elements> 0."
                );
            } else {
                return Loggable<rerun::encodings::Bool>::to_arrow(
                    &instances->interactive,
                    num_instances
                );
            }
        }
    };
} // namespace rerun
