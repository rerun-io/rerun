...
namespace rerun::components {
    ...
    struct ImagePlaneDistance {
        rerun::encodings::Float32 image_from_camera;

      public:
        ImagePlaneDistance() = default;

        ImagePlaneDistance(rerun::encodings::Float32 image_from_camera_)
            : image_from_camera(image_from_camera_) {}

        ImagePlaneDistance& operator=(rerun::encodings::Float32 image_from_camera_) {
            image_from_camera = image_from_camera_;
            return *this;
        }
...
namespace rerun {
    static_assert(sizeof(rerun::encodings::Float32) == sizeof(components::ImagePlaneDistance));

    /// \private
    template <>
    struct Loggable<components::ImagePlaneDistance> {
        static constexpr std::string_view ComponentType = "rerun.components.ImagePlaneDistance";
