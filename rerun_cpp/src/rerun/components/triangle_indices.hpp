...
// Based on "crates/build/re_type_definitions/rerun/components/triangle_indices.def.rs".
...
namespace rerun::components {
    ...
    struct TriangleIndices {
        rerun::encodings::UVec3D indices;
      ...
      public:
        ...
        TriangleIndices(rerun::encodings::UVec3D indices_) : indices(indices_) {}

        TriangleIndices& operator=(rerun::encodings::UVec3D indices_) {
            indices = indices_;
            return *this;
        }
        ...
        /// Cast to the underlying UVec3D encoding
        operator rerun::encodings::UVec3D() const {
            return indices;
        }
    };
} // namespace rerun::components
...
namespace rerun {
    static_assert(sizeof(rerun::encodings::UVec3D) == sizeof(components::TriangleIndices));

    /// \private
    template <>
    struct Loggable<components::TriangleIndices> {
        static constexpr std::string_view ComponentType = "rerun.components.TriangleIndices";

        /// Returns the arrow data type this type corresponds to.
        static const std::shared_ptr<arrow::DataType>& arrow_data_type() {
            return Loggable<rerun::encodings::UVec3D>::arrow_data_type();
        }
        ...
        static Result<std::shared_ptr<arrow::Array>> to_arrow(
            ...
            if (num_instances == 0) {
                return Loggable<rerun::encodings::UVec3D>::to_arrow(nullptr, 0);
                ...
                return Loggable<rerun::encodings::UVec3D>::to_arrow(
                    &instances->indices,
                    num_instances
                );
            }
