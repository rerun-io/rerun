...
namespace rerun {
    const std::shared_ptr<arrow::DataType>& Loggable<components::LineStrip2D>::arrow_data_type() {
        static const auto datatype = arrow::list(
            arrow::field("item", Loggable<rerun::encodings::Vec2D>::arrow_data_type(), false)
        );
        return datatype;
    }
    ...
    rerun::Error Loggable<components::LineStrip2D>::fill_arrow_array_builder(
        ...
        for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
            ...
            if (element.points.data()) {
                RR_RETURN_NOT_OK(Loggable<rerun::encodings::Vec2D>::fill_arrow_array_builder(
                    value_builder,
                    element.points.data(),
                    element.points.size()
                ));
