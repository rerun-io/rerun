...
namespace rerun::components {
    ...
    Result<rerun::DataCell> Scalar::to_data_cell(const Scalar* instances, size_t num_instances) {
        ...
        auto builder_result = Scalar::new_arrow_array_builder(pool);
        RR_RETURN_NOT_OK(builder_result.error);
        auto builder = std::move(builder_result.value);
        if (instances && num_instances > 0) {
            RR_RETURN_NOT_OK(
                Scalar::fill_arrow_array_builder(builder.get(), instances, num_instances)
            );
        }
        std::shared_ptr<arrow::Array> array;
        ARROW_RETURN_NOT_OK(builder->Finish(&array));

        return rerun::DataCell::create(Scalar::NAME, Scalar::arrow_datatype(), std::move(array));
    }
} // namespace rerun::components
