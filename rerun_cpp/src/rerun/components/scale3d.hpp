...
// Based on "crates/build/re_type_definitions/rerun/components/scale3d.def.rs".
    ...
    struct Scale3D {
      ...
      public: // START of extensions from scale3d_ext.cpp:
        ...
        explicit Scale3D(float uniform_scale)
            : Scale3D(encodings::Vec3D{uniform_scale, uniform_scale, uniform_scale}) {}
        ...
        static Scale3D three_d(encodings::Vec3D scale) {
      ...
      public:
        ...
        Scale3D(rerun::encodings::Vec3D scale_) : scale(scale_) {}

        Scale3D& operator=(rerun::encodings::Vec3D scale_) {
            scale = scale_;
            return *this;
        }
        ...
        /// Cast to the underlying Vec3D encoding
        operator rerun::encodings::Vec3D() const {
            return scale;
        }
    };
} // namespace rerun::components
...
namespace rerun {
    static_assert(sizeof(rerun::encodings::Vec3D) == sizeof(components::Scale3D));

    /// \private
    template <>
    struct Loggable<components::Scale3D> {
        static constexpr std::string_view ComponentType = "rerun.components.Scale3D";

        /// Returns the arrow data type this type corresponds to.
        static const std::shared_ptr<arrow::DataType>& arrow_data_type() {
            return Loggable<rerun::encodings::Vec3D>::arrow_data_type();
        }
        ...
        static Result<std::shared_ptr<arrow::Array>> to_arrow(
            ...
            if (num_instances == 0) {
                return Loggable<rerun::encodings::Vec3D>::to_arrow(nullptr, 0);
                ...
                return Loggable<rerun::encodings::Vec3D>::to_arrow(
                    &instances->scale,
                    num_instances
                );
            }
