...
namespace rerun {
    ...
    Result<std::shared_ptr<arrow::Array>> Loggable<encodings::TensorData>::to_arrow(
        const encodings::TensorData* instances, size_t num_instances
    ) {
        ...
        if (instances && num_instances > 0) {
            RR_RETURN_NOT_OK(Loggable<encodings::TensorData>::fill_arrow_array_builder(
    ...
    rerun::Error Loggable<encodings::TensorData>::fill_arrow_array_builder(
        arrow::StructBuilder* builder, const encodings::TensorData* elements, size_t num_elements
    ) {
        if (builder == nullptr) {
        ...
        }
        {
            ...
            ARROW_RETURN_NOT_OK(field_builder->Reserve(static_cast<int64_t>(num_elements)));
            for (size_t elem_idx = 0; elem_idx < num_elements; elem_idx += 1) {
                RR_RETURN_NOT_OK(Loggable<rerun::encodings::TensorBuffer>::fill_arrow_array_builder(
                    field_builder,
