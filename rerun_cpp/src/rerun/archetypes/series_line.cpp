...
namespace rerun::archetypes {
    SeriesLine SeriesLine::clear_fields() {
        ...
        archetype.component_name =
            ComponentBatch::empty<rerun::components::Text>(Descriptor_component_name)
                .value_or_throw();
    ...
    Collection<ComponentColumn> SeriesLine::columns(const Collection<uint32_t>& lengths_) {
        ...
        if (component_name.has_value()) {
            columns.push_back(component_name.value().partitioned(lengths_).value_or_throw());
        }
    ...
    Collection<ComponentColumn> SeriesLine::columns() {
        ...
        if (component_name.has_value()) {
            return columns(std::vector<uint32_t>(component_name.value().length(), 1));
        }
        if (aggregation_policy.has_value()) {
...
namespace rerun {
    ...
    Result<Collection<ComponentBatch>> AsComponents<archetypes::SeriesLine>::as_batches(
        ...
        if (archetype.component_name.has_value()) {
            cells.push_back(archetype.component_name.value());
        }
