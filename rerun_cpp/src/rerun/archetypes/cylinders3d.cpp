...
namespace rerun::archetypes {
    Cylinders3D Cylinders3D::clear_fields() {
        auto archetype = Cylinders3D();
        archetype.lengths =
    ...
    Collection<ComponentColumn> Cylinders3D::columns(const Collection<uint32_t>& lengths_) {
        ...
        columns.push_back(
            ComponentColumn::from_indicators<Cylinders3D>(static_cast<uint32_t>(lengths_.size()))
                .value_or_throw()
        );
        return columns;
    }

    Collection<ComponentColumn> Cylinders3D::columns() {
        if (lengths.has_value()) {
...
namespace rerun {
    ...
    Result<Collection<ComponentBatch>> AsComponents<archetypes::Cylinders3D>::as_batches(
        ...
        {
            auto result = ComponentBatch::from_indicator<Cylinders3D>();
