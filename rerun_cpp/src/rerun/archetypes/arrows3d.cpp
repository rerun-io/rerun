...
namespace rerun {
    ...
    Result<std::vector<DataCell>> AsComponents<archetypes::Arrows3D>::serialize(
        ...
        {
            auto result = DataCell::from_loggable<rerun::components::Vector3D>(archetype.vectors);
            RR_RETURN_NOT_OK(result.error);
            cells.push_back(std::move(result.value));
        }
        if (archetype.origins.has_value()) {
            auto result =
                DataCell::from_loggable<rerun::components::Position3D>(archetype.origins.value());
            RR_RETURN_NOT_OK(result.error);
            cells.push_back(std::move(result.value));
        }
        ...
        }
        {
            ...
            auto result = Loggable<Arrows3D::IndicatorComponent>::to_arrow(&indicator, 1);
            RR_RETURN_NOT_OK(result.error);
