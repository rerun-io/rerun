// DO NOT EDIT! This file was auto-generated by crates/build/re_types_builder/src/codegen/cpp/mod.rs
// Based on "crates/build/re_type_definitions/rerun/blueprint/components/tensor_dimension_index_slider.def.rs".

#pragma once

#include "../../blueprint/encodings/tensor_dimension_index_slider.hpp"
#include "../../result.hpp"

#include <cstdint>
#include <memory>

namespace rerun::blueprint::components {
    /// **Component**: Show a slider for the index of some dimension of a slider.
    ///
    /// ⚠ **This type is _unstable_ and may change significantly in a way that the data won't be backwards compatible.**
    ///
    struct TensorDimensionIndexSlider {
        rerun::blueprint::encodings::TensorDimensionIndexSlider selection;

      public:
        TensorDimensionIndexSlider() = default;

        TensorDimensionIndexSlider(
            rerun::blueprint::encodings::TensorDimensionIndexSlider selection_
        )
            : selection(selection_) {}

        TensorDimensionIndexSlider& operator=(
            rerun::blueprint::encodings::TensorDimensionIndexSlider selection_
        ) {
            selection = selection_;
            return *this;
        }

        TensorDimensionIndexSlider(uint32_t dimension_) : selection(dimension_) {}

        TensorDimensionIndexSlider& operator=(uint32_t dimension_) {
            selection = dimension_;
            return *this;
        }

        /// Cast to the underlying TensorDimensionIndexSlider encoding
        operator rerun::blueprint::encodings::TensorDimensionIndexSlider() const {
            return selection;
        }
    };
} // namespace rerun::blueprint::components

namespace rerun {
    static_assert(
        sizeof(rerun::blueprint::encodings::TensorDimensionIndexSlider) ==
        sizeof(blueprint::components::TensorDimensionIndexSlider)
    );

    /// \private
    template <>
    struct Loggable<blueprint::components::TensorDimensionIndexSlider> {
        static constexpr std::string_view ComponentType =
            "rerun.blueprint.components.TensorDimensionIndexSlider";

        /// Returns the arrow data type this type corresponds to.
        static const std::shared_ptr<arrow::DataType>& arrow_data_type() {
            return Loggable<
                rerun::blueprint::encodings::TensorDimensionIndexSlider>::arrow_data_type();
        }

        /// Serializes an array of `rerun::blueprint:: components::TensorDimensionIndexSlider` into an arrow array.
        static Result<std::shared_ptr<arrow::Array>> to_arrow(
            const blueprint::components::TensorDimensionIndexSlider* instances, size_t num_instances
        ) {
            if (num_instances == 0) {
                return Loggable<rerun::blueprint::encodings::TensorDimensionIndexSlider>::to_arrow(
                    nullptr,
                    0
                );
            } else if (instances == nullptr) {
                return rerun::Error(
                    ErrorCode::UnexpectedNullArgument,
                    "Passed array instances is null when num_elements> 0."
                );
            } else {
                return Loggable<rerun::blueprint::encodings::TensorDimensionIndexSlider>::to_arrow(
                    &instances->selection,
                    num_instances
                );
            }
        }
    };
} // namespace rerun
