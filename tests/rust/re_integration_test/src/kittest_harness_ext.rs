...
pub trait HarnessExt<'h> {
    ...
    // Runs a function with the `AppContext` generated by the actual Rerun application.
    // Prefer this over `run_with_viewer_context` unless you genuinely need the `ViewerContext`.
...
impl<'h> HarnessExt<'h> for egui_kittest::Harness<'h, re_viewer::App> {
    ...
    fn run_with_app_context<R: 'static>(
        ...
        let result_clone = Arc::clone(&result);
        self.state_mut()
            .testonly_set_app_test_hook(Box::new(move |app_context| {
                *result_clone.lock() = Some(func(app_context));
            }));
        self.run_ok();
        result
            .lock()
            .take()
            .expect("app test hook should have been called")
    }
    ...
    fn cursor_icon(&mut self) -> egui::CursorIcon {
        self.run_with_app_context(|app_context| app_context.egui_ctx.output(|o| o.cursor_icon))
    }
