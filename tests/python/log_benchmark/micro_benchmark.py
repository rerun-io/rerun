...
from rerun._baseclasses import BaseBatch, DescribedComponentBatch
...
def main():
    ...
    mat_batch = TransformMat3x3Batch(mat3x3_np)
    ...
    bench("Transform3D(translation=list, mat3x3=np)",
          lambda: rr.Transform3D(translation=translation_list, mat3x3=mat3x3_np))

    bench("Transform3D(translation=np, mat3x3=np)",
          lambda: rr.Transform3D(translation=translation_np, mat3x3=mat3x3_np))
    ...
    bench("Translation3DBatch(list)",
          lambda: Translation3DBatch(translation_list))

    bench("Translation3DBatch(np.array)",
          lambda: Translation3DBatch(translation_np))

    bench("TransformMat3x3Batch(np.eye(3))",
          lambda: TransformMat3x3Batch(mat3x3_np))
    ...
    bench("Vec3D: flat_np_float32 + pa.FixedSizeListArray",
          lambda: pa.FixedSizeListArray.from_arrays(
              np.ascontiguousarray(np.asarray(translation_list, dtype=np.float32).ravel()),
              type=vec3d_type))

    bench("Vec3D: pa.FixedSizeListArray from pre-made np",
    ...
    bench("Mat3x3: pa.FixedSizeListArray from pre-made np",
          lambda: pa.FixedSizeListArray.from_arrays(flat_mat, type=mat_type))

    bench("Mat3x3: full conversion (asarray+reshape+transpose+ravel+from_arrays)",
          lambda: pa.FixedSizeListArray.from_arrays(
              np.ascontiguousarray(np.asarray(mat3x3_np, dtype=np.float32).reshape(-1, 3, 3).transpose(0, 2, 1).reshape(-1)),
              type=mat_type))
    ...
    bench("catch_and_log_exceptions (context mgr, no-op body)",
          lambda: _noop_with_catch())

    bench("try/finally (baseline for comparison)",
          lambda: _noop_try_finally())
    ...
    bench("transform.as_component_batches()",
          lambda: transform.as_component_batches())
    ...
    bench("_log_components(path, batches)",
          lambda: _log_components("test_entity", batches))
    ...
    bench("bindings.log_arrow_msg(path, components)",
    ...
    bench("rr.log(path, pre-built transform)",
          lambda: rr.log("test_entity", transform))

    bench("rr.log(path, Transform3D(...))",
    ...
    bench("rr.set_time('frame', sequence=42)",
          lambda: rr.set_time("frame", sequence=42))
    ...
    bench("ComponentDescriptor('Transform3D:translation', ...)",
          lambda: ComponentDescriptor("Transform3D:translation",
                                       component_type="rerun.components.Translation3D",
                                       archetype="rerun.archetypes.Transform3D"))

    bench("DescribedComponentBatch(batch, descr)",
          lambda: DescribedComponentBatch(trans_batch, descriptors[0]))

    bench("np.asarray(list, float32)",
          lambda: np.asarray(translation_list, dtype=np.float32))

    bench("np.asarray(np_arr, float32) [no-op]",
          lambda: np.asarray(translation_np, dtype=np.float32))

    bench("attrs converter: Translation3DBatch._converter(None)",
          lambda: Translation3DBatch._converter(None))

    bench("attrs converter: Translation3DBatch._converter(list)",
          lambda: Translation3DBatch._converter(translation_list))
    ...
    bench("flat_np_float32_array_from_array_like(list, 3)",
          lambda: flat_np_float32_array_from_array_like(translation_list, 3))

    bench("flat_np_float32_array_from_array_like(np, 3)",
          lambda: flat_np_float32_array_from_array_like(translation_np, 3))

    # The full _native_to_pa_array path for Vec3D
    bench("Translation3DBatch._native_to_pa_array(list, type)",
          lambda: Translation3DBatch._native_to_pa_array(translation_list, vec3d_type))

    bench("Translation3DBatch._native_to_pa_array(np, type)",
    ...
    bench("BaseBatch.__init__ via Translation3DBatch(list)",
          lambda: Translation3DBatch(translation_list))

    # What does isinstance(data, pa.Array) cost?
    bench("isinstance(list, pa.Array)",
          lambda: isinstance(translation_list, pa.Array))

    bench("isinstance(pa_array, pa.Array)",
          lambda: isinstance(arrow_arrays[0], pa.Array))
    ...
    bench("build instanced dict from batches",
          lambda: _build_instanced_dict(batches))

    bench("list comprehension: [b.component_descriptor() for b in batches]",
          lambda: [b.component_descriptor() for b in batches])

    bench("list comprehension: [b.as_arrow_array() for b in batches]",
          lambda: [b.as_arrow_array() for b in batches])
    ...
    bench("__attrs_init__(all None except translation+mat3x3)",
    ...
    bench("6x _converter(None) calls",
          lambda: (
