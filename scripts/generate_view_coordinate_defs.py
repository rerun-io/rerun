...
CPP_EXTENSION = "rerun_cpp/src/rerun/archetypes/view_coordinates_ext.cpp"
...
def gen_cpp_declarations() -> list[str]:
    ...
    lines.extend(cpp_declaration(v) for v in generate_up_handed_permuations())
    lines.append(f"// {END_MARKER.format(key)}\n")
    lines = [" " * 4 + line for line in lines]
    return lines
...
def gen_cpp_definitions() -> list[str]:
    ...
    lines.extend(cpp_definition(v) for v in generate_up_handed_permuations())
    lines.append(f"// {END_MARKER.format(key)}\n")
    lines = [" " * 4 + line for line in lines]
    return lines
...
def main() -> None:
    ...
    if args.rust:
        if args.preview:
            ...
            patch_file(RUST_EXTENSION, gen_rust_code(), "definitions")

    if args.python:
        if args.preview:
            print("".join(gen_py_code()))
        else:
            patch_file(PYTHON_EXTENSION, gen_py_code(), "definitions")

    if args.cpp:
        if args.preview:
            print("DECLARATIONS")
            print("".join(gen_cpp_declarations()))
            print("DEFINITIONS")
            print("".join(gen_cpp_definitions()))
        else:
            patch_file(CPP_EXTENSION, gen_cpp_declarations(), "declarations")
            patch_file(CPP_EXTENSION, gen_cpp_definitions(), "definitions")
