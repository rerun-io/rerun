...
use log_types::*;
use objectron::protos::{ArCamera, ArPlaneAnchor};
...
fn main() {
    ...
    let handle = std::thread::spawn(move || {
        for dir in &args.dirs {
            log_dataset(dir, &rerun_tx).unwrap();
        }
    });
...
fn log_dataset(path: &Path, tx: &Sender<LogMsg>) -> anyhow::Result<()> {
...
fn log_annotation_pbdata(path: &Path, tx: &Sender<LogMsg>) -> anyhow::Result<()> {
    ...
    let sequence = objectron::protos::Sequence::decode(file.as_slice())?;
    let world_space = ObjectPath::from("world");
    ...
    for object in &sequence.objects {
        ...
        let object_path = ObjectPath::from(persist_id("object", Identifier::U64(object.id as _)));

        // dbg!(&object.category); // Most cups have "chair" as the category

        if object.r#type == objectron::protos::object::Type::BoundingBox as i32 {
    ...
    for frame_annotation in sequence.frame_annotations.iter() {
        ...
        for object_annotation in &frame_annotation.annotations {
            let object_path = ObjectPath::from(persist_id(
                "object",
                Identifier::U64(object_annotation.object_id as _),
            ));
            ...
            if keypoints_2d.len() == 9 {
                ...
                for (id, pos2) in keypoint_ids.into_iter().zip(keypoints_2d) {
                    let point_path = &object_path / persist_id("point", id as u64);
                    tx.send(
...
fn log_geometry_pbdata(path: &Path, tx: &Sender<LogMsg>) -> anyhow::Result<()> {
    ...
    while reader.read_exact(&mut msg_len).is_ok() {
        ...
        use prost::Message as _;
        let ar_frame = objectron::protos::ArFrame::decode(msg.as_slice())?;
            ...
            if let Some(count) = raw_feature_points.count {
                ...
                for i in 0..count as usize {
                    ...
                    if let (Some(x), Some(y), Some(z)) = (point.x, point.y, point.z) {
                        let point_path = &points_path / persist_id("point", identifier);
...
fn log_plane_anchor(
    ...
    let plane_path = root_path / persist_id("plane", identifier);
